sap.ui.define([
    "sap/ui/core/mvc/Controller",
    "sap/ui/model/json/JSONModel",
    "sap/f/library",
    "sap/ui/layout/HorizontalLayout",
    "sap/ui/layout/VerticalLayout",
    "sap/ui/core/BusyIndicator",
    "sap/ui/model/Filter",
    "sap/ui/model/FilterOperator",
    "sap/m/MessageBox",
    "sap/m/MessageToast",
    "sap/ui/core/Fragment",
    "sap/m/MenuItem",
    "sap/m/Menu",
    "sap/m/ColumnListItem",
    "sap/m/Text",
    "sap/m/ComboBox",
    "sap/m/Input",
    "sap/m/VBox",
    "sap/m/Select"



], (Controller, JSONModel, fioriLibrary, HorizontalLayout, VerticalLayout, BusyIndicator, Filter, FilterOperator, MessageBox, MessageToast, Fragment, MenuItem, Menu, ColumnListItem, Text, ComboBox, Input, VBox, Select) => {
    "use strict";


    return Controller.extend("com.taqa.psnform.taqapsnform.controller.PSNForm", {

        onInit() {
            BusyIndicator.show(0);

            this.oFlexibleColumnLayout = this.byId("flexibleColumnLayout");

            let data = this.getOwnerComponent().getModel("DataModel")
            this.getView().setModel(data, "DataModel");
            // let ListDataModel = new JSONModel();
            // this.getView().setModel(ListDataModel, "ListData");

            let oListDataModel = new JSONModel({}); // Create an empty JSON model
            this.getView().setModel(oListDataModel, "ListData"); // Set the ListData model

            this.oFlexibleColumnLayout = this.byId("flexibleColumnLayout");
            this.oViewModel = new sap.ui.model.json.JSONModel({
                showExitFullScreen: false,
                showFullScreen: true,
                showSearchSort: false,
                showRaiseRequest: true
            });
            this.getView().setModel(this.oViewModel, "buttonModel");

            this.oEmployeeSearchModel = new JSONModel({
                employees: []
            });
            this.getView().setModel(this.oEmployeeSearchModel, "employeeSearch");

            this.changeApprovalModel = new JSONModel({ employees: [] });
            this.getView().setModel(this.changeApprovalModel, "changeApproval");

            this.oEventReasonsModel = new JSONModel({
                eventReasons: []
            });
            this.getView().setModel(this.oEventReasonsModel, "eventReasons");

            this.oFlexibleColumnLayout.attachStateChange(this.onLayoutChange, this);

            const loadPromises = [
                new Promise(resolve => {
                    this._loadTypeofChangePicklist();
                    resolve();
                }),
                new Promise(resolve => {
                    this._getPendingListDetails();
                    resolve();
                })
            ];

            this._selectedEmployeeId = "";
            this._selectedItemContext = null;

            this.oSelectedRowModel = new JSONModel();
            this.getView().setModel(this.oSelectedRowModel, "selectedRowModel");


            let oSelectedEmployeeModel = new sap.ui.model.json.JSONModel({
                selectedEmployee: {}
            });

            this.getView().setModel(oSelectedEmployeeModel, "selectedEmployeeModel");

            const delayPromise = new Promise(resolve => {
                setTimeout(resolve, 1000);
            });

            Promise.all([...loadPromises, delayPromise])
                .then(() => {
                    BusyIndicator.hide();
                })
                .catch(error => {
                    console.error("Error during initialization:", error);
                    BusyIndicator.hide();
                });

            // sap.ui.require(["sap/ushell/Container"], async function (Container) {
            //     const UserInfo = await Container.getServiceAsync("UserInfo");
            //     let oUserModel = new sap.ui.model.json.JSONModel({
            //         email: UserInfo.getEmail(),   
            //         firstName: UserInfo.getFirstName(),
            //         lastName: UserInfo.getLastName(),
            //         fullName: UserInfo.getFullName(),
            //         id: UserInfo.getId()
            //     });
            //     this.getView().setModel(oUserModel, "userModel");
            //     if (UserInfo) {
            //         console.log("User Email ID:", UserInfo.getEmail());
            //     } else {
            //         console.log("Email ID not available");
            //     }
            // }.bind(this));

            this.oUserModel = new JSONModel({
                userDetails: {}
            });
            this.getView().setModel(this.oUserModel, "view");

            this._getUserName();

            this.oButtonModel = new sap.ui.model.json.JSONModel({
                showSubmitButton: false
            });
            this.getView().setModel(this.oButtonModel, "buttonState");


            var oModel = new JSONModel({
				senderEmail: "",
				events: [
					{
						title: "Invoice Approval Required",
						description: "Invoice #INV-2023-001 requires your approval",
						date: "2025-05-14",
						recipientEmail: "finance@example.com",
						emailSubject: "Action Required: Invoice #INV-2023-001 Approval",
						emailBody: "Dear Finance Team,\n\nAn invoice (#INV-2023-001) requires your approval. Please review and approve by May 16th, 2025.\n\nRegards,\nSAP System"
					},
					{
						title: "Purchase Order Created",
						description: "PO #PO2025-0042 has been created",
						date: "2025-05-13",
						recipientEmail: "supplier@example.com",
						emailSubject: "Purchase Order #PO2025-0042 Notification",
						emailBody: "Dear Supplier,\n\nA new purchase order (#PO2025-0042) has been created for your company. Please review the attached document and confirm receipt.\n\nBest regards,\nProcurement Team"
					},
					{
						title: "Delivery Delay Alert",
						description: "Shipment #SH-8842 is delayed",
						date: "2025-05-12",
						recipientEmail: "logistics@example.com",
						emailSubject: "URGENT: Delivery Delay for Shipment #SH-8842",
						emailBody: "Dear Logistics Team,\n\nThis is to inform you that shipment #SH-8842 will be delayed by 2 days. Please update the delivery schedule accordingly.\n\nRegards,\nSupply Chain Management"
					}
				]
			});
			this.getView().setModel(oModel);
			
			// Get the logged-in user's email
			this._getUserEmail();
		},
		
		_getUserEmail: function() {
			var that = this;
			
			sap.ui.require(["sap/ushell/services/Container"], function (Container) {
				try {
					var oUser = sap.ushell.Container.getService("UserInfo");
					var sEmail = oUser.getEmail();
					
					that.getView().getModel().setProperty("/senderEmail", sEmail);
				} catch (e) {
					console.log("Could not retrieve user email: " + e);
					that.getView().getModel().setProperty("/senderEmail", "Manasa.Arella@cytechies.com");
				}
			}, function() {
				that.getView().getModel().setProperty("/senderEmail", "Manasa.Arella@cytechies.com");
			});
		},
		
		onSendEmailPressed: function(oEvent) {

			var oButton = oEvent.getSource();
			var sRecipientEmail = oButton.data("recipientEmail");
			var sSubject = oButton.data("emailSubject");
			var sBody = oButton.data("emailBody");
			var sSenderEmail = this.getView().getModel().getProperty("/senderEmail");
			
			this._currentEmail = {
				senderEmail: sSenderEmail,
				recipientEmail: sRecipientEmail,
				subject: sSubject,
				body: sBody
			};
			
			var sEmailPreview = 
				"From: " + sSenderEmail + "\n" +
				"To: " + sRecipientEmail + "\n" +
				"Subject: " + sSubject + "\n\n" +
				"---------- Email Body ----------\n" +
				sBody + 
				"\n-------------------------------";
			
			var that = this;
			MessageBox.confirm(
				sEmailPreview, 
				{
					title: "Send Email Confirmation",
					actions: [MessageBox.Action.OK, MessageBox.Action.CANCEL],
					emphasizedAction: MessageBox.Action.OK,
					onClose: function(sAction) {
						if (sAction === MessageBox.Action.OK) {
							that._sendEmail();
						}
					}
				}
			);
		},
		
		_sendEmail: function() {	
			sap.ui.core.BusyIndicator.show(0);
			
			var that = this;
			var oEmail = this._currentEmail;
		
			jQuery.ajax({
				url: this.getPath() + "/email/send",  
				type: "POST",
				data: JSON.stringify(oEmail),
				contentType: "application/json",
				success: function(data) {
					// Hide loading indicator
					sap.ui.core.BusyIndicator.hide();
					
					// Show success message
					MessageBox.success("Email sent successfully to " + oEmail.recipientEmail);
				},
				error: function(jqXHR, textStatus, errorThrown) {
					// Hide loading indicator
					sap.ui.core.BusyIndicator.hide();
					
					// Show error message
					MessageBox.error("Failed to send email: " + errorThrown);
				}
			});
			
			setTimeout(function() {
				// Hide loading indicator
				sap.ui.core.BusyIndicator.hide();
				
				// Show success message
				MessageToast.show("Email sent successfully to " + oEmail.recipientEmail);
			}, 1500);
		},


        formatButtonVisibility: function (status) {
            return status === "REQUEST COMPLETED";
        },

        onNewRequestDialog: function () {
            let route = this.getOwnerComponent().getRouter();
            route.navTo("RouteNewForm", { action: "openFragment" });

        },

        // dynamicButtonsEnable: function () {

        //     var oModel = this.getOwnerComponent().getModel("DataModel");
        //     this.getView().setModel(oModel, "buttonModel");

        //     oModel.setProperty("/ButtonVisibility/currentCondition", "C1");
        //     var currentCondition = oModel.getProperty("/ButtonVisibility/currentCondition");
        //     var currentObject = oModel.getProperty("/ButtonVisibility/conditions/" + currentCondition);


        //     oModel.setProperty("/ButtonVisibility/currentCondition", currentObject);


        //     console.log("Current Condition Key:", currentCondition);
        //     console.log("Current Condition Object:", currentObject);

        // },


        // 14-05-2025 Code

      


        // onListItemPress: function (oEvent) {
        //     let oItem = oEvent.getParameter("listItem"); 
        //     let oModel = this.getView().getModel("ListData");
        //     let sPath = oItem.getBindingContextPath();
        //     let oSelectedRowData = oModel.getProperty(sPath);
        //     this.byId("submitUpdateButton").setVisible(true);

        //     console.log("Selected row data:", oSelectedRowData);
        //     // Log the event type specifically
        //     console.log("Event type:", oSelectedRowData?.cust_PSNTypeChange);

        //     BusyIndicator.show(0);

        //     let userId = oSelectedRowData.externalCode;
        //     console.log(userId, typeof userId);


        //     try {
        //         let busyPromise = new Promise(function (resolve) {
        //             setTimeout(function () {
        //                 resolve();
        //             }, 100);
        //         });
        //         busyPromise.then(function () {

        //             this.dynamicButtonsEnable();

        //             this._getDetails(userId);
        //             this._getEducationDetails(userId);
        //             this._getLastExpDetails(userId);
        //             this._getSalaryAdjustDetails(userId);
        //             this._getApprovalDetails(userId);
        //             this.fetchRequestApprovalData(userId);
        //             this._getWorkflowDetails();


        //             this.oSelectedRowModel.setData({
        //                 selectedRow: oSelectedRowData
        //             });

        //             this.disableChangeOfStatusSection();
        //             this.disableChangeOfCompSection();
        //             this.disableSubmitApprovalsSection();
        //             this.oButtonModel.setProperty("/showSubmitButton", false);



        //             console.log("Entire Model Data:", this.oSelectedRowModel.getData());

        //             let status = oSelectedRowData?.wfRequestNav?.results?.[0]?.status;

        //             // this._highlightSectionsOnComplete(status);


        //             this.oFlexibleColumnLayout.setLayout(fioriLibrary.LayoutType.TwoColumnsMidExpanded);
        //             this.oViewModel.setProperty("/showSearchSort", true);
        //             this.oViewModel.setProperty("/showRaiseRequest", false);

        //             let oMidColumnPage = this.byId("ObjectPageLayout");
        //             oMidColumnPage.bindElement({
        //                 path: sPath,
        //                 model: "ListData"
        //             });
        //         }.bind(this)).catch(function (error) {
        //             console.error("Error during processing: ", error);
        //         }).finally(function () {
        //             BusyIndicator.hide();

        //         }.bind(this));
        //     } catch (error) {
        //         console.error("Error during processing: ", error);
        //         BusyIndicator.hide();
        //     }

        //     this._selectedItemContext = oItem.getBindingContext("ListData"); // Use the selected item's context
        //     this._bSortAscending = true;


        // },

        // 19/05/2025

        onListItemPress: function (oEvent) {
            let oItem = oEvent.getParameter("listItem");
            let oModel = this.getView().getModel("ListData");
            let sPath = oItem.getBindingContextPath();
            let oSelectedRowData = oModel.getProperty(sPath);
            this.byId("submitUpdateButton").setVisible(true);
        
            BusyIndicator.show(0);
        
            let userId = oSelectedRowData.externalCode;
            let eventType = oSelectedRowData?.cust_PSNTypeChange;
            console.log("Event type:", eventType);
        
        
            try {
                let busyPromise = new Promise(function (resolve) {
                    setTimeout(function () {
                        resolve();
                    }, 100);
                });
        
                busyPromise.then(function () {

                    this.dynamicButtonsEnable(eventType);
        
                    this._getDetails(userId);
                    this._getEducationDetails(userId);
                    this._getLastExpDetails(userId);
                    this._getSalaryAdjustDetails(userId);
                    this._getApprovalDetails(userId);
                    this.fetchRequestApprovalData(userId);
                    this._getWorkflowDetails();
        
                    this.oSelectedRowModel.setData({
                        selectedRow: oSelectedRowData
                    });
        
                    this.disableChangeOfStatusSection();
                    this.disableChangeOfCompSection();
                    this.disableSubmitApprovalsSection();
                    this.oButtonModel.setProperty("/showSubmitButton", false);
        
                    let status = oSelectedRowData?.wfRequestNav?.results?.[0]?.status;
                    this.oFlexibleColumnLayout.setLayout(fioriLibrary.LayoutType.TwoColumnsMidExpanded);
                    this.oViewModel.setProperty("/showSearchSort", true);
                    this.oViewModel.setProperty("/showRaiseRequest", false);
        
                    let oMidColumnPage = this.byId("ObjectPageLayout");
                    oMidColumnPage.bindElement({
                        path: sPath,
                        model: "ListData"
                    });
                }.bind(this)).catch(function (error) {
                    console.error("Error during processing: ", error);
                }).finally(function () {
                    BusyIndicator.hide();
                }.bind(this));
            } catch (error) {
                console.error("Error during processing: ", error);
                BusyIndicator.hide();
            }
        
            this._selectedItemContext = oItem.getBindingContext("ListData");
            this._bSortAscending = true;

            BusyIndicator.hide();
        },

        dynamicButtonsEnable: function () {
            var oButtonModel = this.getOwnerComponent().getModel("DataModel");
            this.getView().setModel(oButtonModel, "buttonModel");
            var oSelectedRowData = this.oSelectedRowModel.getProperty("/selectedRow");
            var sEventType = oSelectedRowData?.cust_PSNTypeChange;
        
            if (!sEventType) {
                console.warn("No event type found in selected row.");
                return;
            }
            sEventType = sEventType.replace(/\s+/g, "").trim();
            var oUserModel = this.getView().getModel("UserData");
            var sRole = oUserModel?.getProperty("/userAttributes/role");
        
            if (!sRole) {
                sRole = "DepartmentHead";
                console.warn("User role not found. Defaulting to:", sRole);
            }
        
            var sPath = `/ButtonVisibility/RequestTypes/${sEventType}/${sRole}`;
            var oVisibilityConfig = oButtonModel.getProperty(sPath);
        
            if (!oVisibilityConfig) {
                console.warn("No visibility config found for path:", sPath);
                oVisibilityConfig = {};
            }
            var oJSONModel = new JSONModel(oVisibilityConfig);
            this.getView().setModel(oJSONModel, "buttonModel");
            
            console.log("Event:", sEventType, "| Role:", sRole, "| Config:", sPath, oVisibilityConfig);
            console.log("Event:", sEventType, "| Role:", sRole, "| Config:", sPath , oVisibilityConfig);
        },
        
        
        

        _highlightSectionsOnComplete: function (status) {
            if (status === "COMPLETED") {
                let userId = null;

                if (this._selectedItemContext) {
                    let selectedRowData = this._selectedItemContext.getObject();
                    userId = selectedRowData.externalCode;
                } else if (this.oSelectedRowModel && this.oSelectedRowModel.getData().selectedRow) {
                    userId = this.oSelectedRowModel.getData().selectedRow.externalCode;
                }

                if (!userId) {
                    MessageToast.show("No employee selected. Please select an employee first.");
                    console.error("No user ID available for onUpdatePosition");
                    this.byId("submitUpdateButton").setVisible(true);
                    return;
                }

                console.log("Updating position for user:", userId);

                this.fetchEmpJobData(userId);
                this.fetchEmpCompensationData(userId);
                this.fetchRequestApprovalData(userId);
                this._getEventReasons();
                this._getWorkflowDetails(userId)
                // this.highlightChangeOfStatusSection();
                // this.highlightChangeOfCompSection();
                this.highlightSubmitApprovalsSection();

                if (this.empJobData) {
                    this.initializeView();
                } else {
                    MessageToast.show("Failed to load employee data. Please try again.");
                    this.byId("submitUpdateButton").setVisible(true);
                }
            }
        },

        onSearchIconPress: function () {
            let oSearchField = this.getView().byId("idSearchField");
            let bSearchFieldVisible = oSearchField.getVisible();

            oSearchField.setVisible(!bSearchFieldVisible);

            if (!bSearchFieldVisible) {
                oSearchField.focus();
            }
        },

        onSearch: function (oEvent) {

            let sQuery = oEvent.getParameter("query") || oEvent.getParameter("newValue"); // Use either "query" or "newValue"

            let oList = this.byId("idList");

            let oBinding = oList.getBinding("items");

            if (sQuery && sQuery.length > 0) {
                let oFilter = new sap.ui.model.Filter([
                    new sap.ui.model.Filter("externalCode", sap.ui.model.FilterOperator.Contains, sQuery),
                    new sap.ui.model.Filter("cust_PSNTypeChange", sap.ui.model.FilterOperator.Contains, sQuery)
                ], false);

                oBinding.filter(oFilter);
            } else {

                oBinding.filter([]);
            }
        },

        onSortIconPress: function () {
            let oList = this.getView().byId("page2").getContent()[0]; 
            let oBinding = oList.getBinding("items");
            let bSortAscending = this._bSortAscending; 

            let oSorter = new sap.ui.model.Sorter("externalCode", !bSortAscending);

            oBinding.sort(oSorter);

            this._bSortAscending = !bSortAscending; // Toggle sort order
        },

        onOpenMoreActionsMenu: function (oEvent) {
            // Create the menu if it doesn't already exist
            if (!this._oMoreActionsMenu) {
                this._oMoreActionsMenu = new Menu({
                    items: [
                        new sap.m.MenuItem({
                            text: "Send Email",
                            icon: "sap-icon://nav-back",
                            press: this.onSendEmailPressed.bind(this)
                        }),
                        new MenuItem({
                            text: "Withdraw",
                            icon: "sap-icon://delete",
                            press: this.onWithdraw.bind(this)
                        }),
                        // new sap.m.MenuItem({
                        //     text: "Delegate",
                        //     icon: "sap-icon://user-edit",
                        //     press: this.onToggleFooter.bind(this)
                        // })
                    ]
                });
            }

            // Open the menu at the position of the button
            this._oMoreActionsMenu.openBy(oEvent.getSource());
        },

        _updateButtonVisibilityAfterApproval: function () {

            let oSubmitButton = this.byId("submitChangesButton");
            if (oSubmitButton) {
                oSubmitButton.setVisible(true);
            }

            // Hide other buttons
            let oApproveButton = this.byId("approveButton");
            let oRejectButton = this.byId("rejectButton");
            let oMoreActionsButton = this.byId("moreActionsButton");

            if (oApproveButton) oApproveButton.setVisible(false);
            if (oRejectButton) oRejectButton.setVisible(false);
            if (oMoreActionsButton) oMoreActionsButton.setVisible(false);

        },

        onToggleFooter: function () {
            MessageToast.show("Delegate action triggered");

        },

        getPath: function (destinationType) {
            let appId = this.getOwnerComponent().getManifestEntry("/sap.app/id");
            let appPath = appId.replaceAll(".", "/");
            let appModulePath = jQuery.sap.getModulePath(appPath);

            if (destinationType === "SF_1") {
                return appModulePath + "/odata/v2/basic";
            } else if (destinationType === "SF_2") {
                return appModulePath + "/odata/v2/sf2";
            } else if (destinationType === "SF_3") {
                return appModulePath + "/odata/v2/sf3";
            } else if (destinationType === "SF_4") {
                return appModulePath + "/odata/v2/sf4";
            } else if (destinationType === "SF_5") {
                return appModulePath + "/odata/v2/sf5";
            } else if (destinationType === "SF_OAUTH") {
                return appModulePath + "/odata/v2/oauth";
            }
            return appModulePath;
        },

        _getUserName: function () {
          
            let sServiceUrl = this.getPath() + "/user-api/currentUser";

            let that = this;
            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {

                    let userModel = new JSONModel(data);
                    that.getView().setModel(userModel, "UserData")

                    console.log("Fetched Data: ", userModel.getData());
                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },


        _getPendingListDetails: function () {
            let that = this;
            let pendingData = [];
            let completedData = [];
            const mergeAndBindData = function () {
                if (pendingData.length > 0 || completedData.length > 0) {
                    const combinedData = {
                        results: [...pendingData, ...completedData]
                    };
                    let listDataModel = that.getView().getModel("ListData");
                    listDataModel.setData(combinedData);
                    if (combinedData && combinedData.results) {
                        combinedData.results.forEach(function (item) {
                            if (item.externalCode) {
                                if (item.recordStatus === "PENDING") {
                                    that._getSubmitWorkflowIds(item.externalCode);
                                } else if (item.recordStatus === "COMPLETED") {
                                    that.fetchRequestApprovalData(item.externalCode);
                                }
                            }
                        });
                    }
                    console.log("Combined List Data:", listDataModel.getData());
                }
            };

            // Get pending records
            let sServiceUrl1 = this.getPath("SF_1") + "/cust_PositionStatusChange?recordStatus=pending&$format=JSON&$select=externalCode,effectiveStartDate,cust_EffectiveDate,cust_EMP_NameNav/displayName,cust_PSNTypeChange,cust_Justification,wfRequestNav/wfRequestId,wfRequestNav/totalSteps,wfRequestNav/currentStepNum,wfRequestNav/status,wfRequestNav/wfRequestStepNav/stepNum,wfRequestNav/wfRequestStepNav/wfRequestStepId,wfRequestNav/wfRequestStepNav/status,wfRequestNav/wfRequestStepNav/positionNav/code,wfRequestNav/wfRequestStepNav/positionNav/externalName_en_US,cust_AttachmentNav/attachmentId,cust_AttachmentNav/fileName&$expand=wfRequestNav/wfRequestStepNav/positionNav,cust_EMP_NameNav,cust_AttachmentNav";

            $.ajax({
                url: sServiceUrl1,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    if (data && data.d && data.d.results) {
                        console.log("Pending Data:", data.d.results);
                        pendingData = data.d.results.map(function (item) {
                            item.recordStatus = "pending"; // Add status for identification
                            return item;
                        });
                        mergeAndBindData();
                    }
                },
                error: function (e) {
                    console.error("Error fetching pending records:", e);
                    MessageToast.show("Error fetching pending records");
                }
            });

            // Get completed records
            let sServiceUrl2 = this.getPath("SF_1") + "/cust_PositionStatusChange?recordStatus=completed&$format=JSON&$expand=cust_AttachmentNav,cust_EMP_NameNav&$select=externalCode,effectiveStartDate,cust_EffectiveDate,cust_EMP_NameNav/displayName,cust_PSNTypeChange,cust_Justification,cust_Emp_ID,cust_EventReason,cust_AttachmentNav/attachmentId,cust_AttachmentNav/fileName,cust_EMP_NameNav/displayName";

            $.ajax({
                url: sServiceUrl2,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    if (data && data.d && data.d.results) {
                        console.log("Completed Data:", data.d.results);
                        completedData = data.d.results.map(function (item) {
                            item.recordStatus = "COMPLETED"; // Add status for identification
                            // Initialize wfRequestNav to avoid binding errors
                            item.wfRequestNav = { results: [] };
                            return item;
                        });
                        mergeAndBindData();
                    }
                },
                error: function (e) {
                    console.error("Error fetching completed records:", e);
                    MessageToast.show("Error fetching completed records");
                }
            });
        },

        fetchRequestApprovalData: function (userId) {
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/EmpWfRequest?$format=json&$filter=subjectId eq '" + userId + "' and requestType eq 'CHANGE_GENERIC_OBJECT'&$select=wfRequestId";

            jQuery.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    if (!data || !data.d || !Array.isArray(data.d.results) || data.d.results.length === 0) {
                        console.warn("No workflow requests found for user ID:", userId);
                        return;
                    }

                    let listDataModel = that.getView().getModel("ListData");
                    let combinedData = listDataModel.getData().results;

                    if (!Array.isArray(combinedData)) {
                        console.error("ListData model 'results' is not an array.");
                        return;
                    }

                    let userItem = combinedData.find(item => String(item.externalCode) === userId);

                    if (!userItem) {
                        console.warn("User item not found for ID:", userId);
                        return;
                    }

                    data.d.results.forEach(function (result) {
                        if (result.wfRequestId) {
                            if (!userItem.wfRequestNav) {
                                userItem.wfRequestNav = { results: [] };
                            }
                            let wfPlaceholder = {
                                wfRequestId: result.wfRequestId,
                                totalSteps: 0,
                                currentStepNum: 0,
                                status: "PENDING",
                                wfRequestStepNav: { results: [] }
                            };

                            userItem.wfRequestNav.results.push(wfPlaceholder);

                            that._fetchWorkflowDetails(result.wfRequestId, userItem);
                        }
                    });

                    listDataModel.refresh();
                },
                error: function (e) {
                    MessageToast.show("Error fetching workflow data for completed items.");
                    console.error("Error fetching completed approval data:", e);
                }
            });
        },

        _fetchWorkflowDetails: function (wfRequestId, listItem) {
            let that = this;

            let sDetailUrl = this.getPath("SF_1") +
                `/WfRequest(${wfRequestId}L)?$format=json&$expand=wfRequestStepNav,wfRequestStepNav/positionNav&$select=wfRequestId,totalSteps,currentStepNum,status,wfRequestStepNav/stepNum,wfRequestStepNav/wfRequestStepId,wfRequestStepNav/status,wfRequestStepNav/positionNav/code,wfRequestStepNav/positionNav/externalName_en_US`;

            $.ajax({
                url: sDetailUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (oDetailData) {
                    if (oDetailData && oDetailData.d) {
                        // Find the matching wfRequestNav entry in the listItem
                        if (listItem.wfRequestNav && Array.isArray(listItem.wfRequestNav.results)) {
                            let wfEntry = listItem.wfRequestNav.results.find(
                                entry => entry.wfRequestId === oDetailData.d.wfRequestId
                            );

                            if (wfEntry) {
                                // Update with detailed information
                                wfEntry.totalSteps = oDetailData.d.totalSteps;
                                wfEntry.currentStepNum = oDetailData.d.currentStepNum;
                                wfEntry.status = oDetailData.d.status;

                                // Process step information - handle positionNav structure
                                if (oDetailData.d.wfRequestStepNav && Array.isArray(oDetailData.d.wfRequestStepNav.results)) {
                                    wfEntry.wfRequestStepNav.results = oDetailData.d.wfRequestStepNav.results.map(function (step) {
                                        // Ensure positionNav is properly formatted for binding
                                        if (step.positionNav) {
                                            // If positionNav doesn't have a results property, wrap it
                                            if (!step.positionNav.results) {
                                                step.positionNav = {
                                                    results: [step.positionNav]
                                                };
                                            }
                                        } else {
                                            step.positionNav = { results: [] };
                                        }
                                        return step;
                                    });
                                }

                                // Mark as completed if applicable
                                if (oDetailData.d.status === "COMPLETED") {
                                    wfEntry.status = "COMPLETED";
                                    listItem.positionStatus = "COMPLETED";
                                    console.log("Workflow COMPLETED for item:", listItem.externalCode);
                                }
                            }
                        }

                        // Update the model
                        that.getView().getModel("ListData").refresh();
                        console.log("Updated workflow details for ID:", wfRequestId);
                    }
                },
                error: function (xhr, status, error) {
                    console.error("Error fetching workflow detail for ID:", wfRequestId);
                    console.error("Status:", status);
                    console.error("Error:", error);
                }
            });
        },


        _getSubmitWorkflowIds: function (userId) {
            let that = this;
            let sListUrl = this.getPath("SF_1") +
                "/EmpWfRequest?$format=json&$filter=subjectId eq '" + userId + "' and eventReason like 'PSN%'&$top=1";

            $.ajax({
                url: sListUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let listDataModel = that.getView().getModel("ListData").getData();
                    let combinedData = listDataModel.results;

                    if (!Array.isArray(combinedData)) {
                        console.error("ListData model 'results' is not an array.");
                        return;
                    }

                    if (!data || !data.d || !Array.isArray(data.d.results)) {
                        console.warn("Fetched workflow request data is empty or invalid.");
                        return;
                    }

                    let wfRequests = data.d.results;

                    combinedData.forEach(function (item) {
                        wfRequests.forEach(function (request) {
                            if (String(item.externalCode) === userId) {
                                if (request.wfRequestId) {
                                    if (!item.wfRequestNav) {
                                        item.wfRequestNav = { results: [] };
                                    }
                                    let wfEntry = {
                                        wfRequestId: request.wfRequestId,
                                        eventReason: request.eventReason,
                                        status: "position submitted"
                                    };

                                    item.wfRequestNav.results.push(wfEntry);
                                    item.positionStatus = "position submitted";
                                    that._fetchWorkflowDetails(request.wfRequestId, item);
                                }
                            }
                        });
                    });
                    that.getView().getModel("ListData").setData({ results: combinedData });
                    let positionWfModel = new sap.ui.model.json.JSONModel({
                        requests: wfRequests
                    });
                    that.getView().setModel(positionWfModel, "positionWfRequests");

                    console.log("✅ Matched position workflow requests with ListData:", combinedData);
                },
                error: function (xhr, status, error) {
                    MessageToast.show("Error fetching position workflow requests.");
                    console.error("Error fetching position workflow requests.");
                    console.error("Status:", status);
                    console.error("Error:", error);
                }
            });
        },

 
        _getDetails: function (userId) {
            // userId = "31120";
            // Construct the complete URL
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/User(" + userId + ")?$select=firstName,lastName,nationality,empId,userId,username,displayName,hireDate,defaultFullName,married,empInfo/jobInfoNav/employmentTypeNav/picklistLabels/optionId,empInfo/jobInfoNav/employmentTypeNav/picklistLabels/locale,empInfo/jobInfoNav/employmentTypeNav/picklistLabels/label&$format=JSON&$expand=empInfo/jobInfoNav,empInfo/jobInfoNav/employmentTypeNav/picklistLabels";


            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let employeeDataModel = new JSONModel(data.d);
                    that.getView().setModel(employeeDataModel, "empData")


                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },


        _getEducationDetails: function (userId) {
            let that = this;
            // userId = "31120";
            let sServiceUrl = this.getPath("SF_1") + "/Background_Education?$format=json&$select=userId,majorNav/picklistLabels/label,majorNav/picklistLabels/locale,majorNav/picklistLabels/optionId,sub_majorNav/picklistLabels/label,sub_majorNav/picklistLabels/locale,sub_majorNav/picklistLabels/optionId,degreeNav/picklistLabels/label,degreeNav/picklistLabels/locale,degreeNav/picklistLabels/optionId,schoolNav/picklistLabels/label,schoolNav/picklistLabels/locale,schoolNav/picklistLabels/optionId&$expand=majorNav/picklistLabels,degreeNav/picklistLabels,sub_majorNav/picklistLabels,schoolNav/picklistLabels&$filter=userId eq '" + userId + "'";


            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let educationDataModel = new JSONModel(data.d);
                    that.getView().setModel(educationDataModel, "educationData");


                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },


        _getApprovalDetails: function () {
            // userId = "31120";
            let that = this;
            let sServiceUrl1 = this.getPath("SF_1") + "/EmpJob?$format=JSON&$select=userId,position,jobTitle,userNav/displayName&$expand=userNav&$filter=(position eq 21000885 or position eq 21000902 or position eq 22000551) and userNav/firstName ne null";
            $.ajax({
                url: sServiceUrl1,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let approvalDataModel = new JSONModel(data.d);
                    that.getView().setModel(approvalDataModel, "approvalData")
                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },


        _getLastExpDetails: function (userId) {
            //userId = "31844";
            // Construct the complete URL
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/Background_OutsideWorkExperience?$format=json&$select=startTitle,endDate,startDate,employer,yearsofexperience&$top=1&$filter=userId eq '" + userId + "'";


            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let lastExpDataModel = new JSONModel(data.d);
                    that.getView().setModel(lastExpDataModel, "lastExpData")



                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },

        _getSalaryAdjustDetails: function (userId) {
            //  userId = "31288";
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/FormHeader?$format=json&$select=dateAssigned,formLastContent/pmReviewContentDetail/summarySection/calculatedFormRating/rating,&$expand=formLastContent/pmReviewContentDetail/summarySection/calculatedFormRating&$filter=formSubjectId eq '" + userId + "' and formDataStatus eq 3";


            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let SalaryAdjustDataModel = new JSONModel(data.d);
                    that.getView().setModel(SalaryAdjustDataModel, "SalaryAdjustData")

                },
                error: function () {
                    MessageToast.show("Server Send Error");
                }
            });

        },

        createRatingRow: function (sId, oContext) {
            let oData = oContext.getObject();
            let sRating = "";
            let sYear = this.formatYear(oData.dateAssigned);

            if (oData.formLastContent &&
                oData.formLastContent.pmReviewContentDetail &&
                oData.formLastContent.pmReviewContentDetail.results &&
                oData.formLastContent.pmReviewContentDetail.results.length > 0 &&
                oData.formLastContent.pmReviewContentDetail.results[0].summarySection &&
                oData.formLastContent.pmReviewContentDetail.results[0].summarySection.calculatedFormRating) {

                sRating = oData.formLastContent.pmReviewContentDetail.results[0].summarySection.calculatedFormRating.rating;
            }

            return new ColumnListItem({
                cells: [
                    new Text({ text: sYear }),
                    new Text({ text: sRating || "No rating" })
                ]
            });
        },



        getApproverNames: function (positionCode) {
            let approvalData = this.getView().getModel("approvalData");
            let approverNames = []; // Array to collect matching approver names

            if (approvalData && approvalData.getProperty("/results") && positionCode) {
                let approvers = approvalData.getProperty("/results");
                // Iterate through all approvers
                approvers.forEach(function (approver) {
                    if (approver.position === positionCode && approver.userNav && approver.userNav.displayName) {
                        approverNames.push(approver.userNav.displayName); // Add matching approver name to the array
                    }
                });
            }
            return approverNames;
        },

        _getEventReasons: function () {
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + `/FOEventReason?$select=event,externalCode,name&$format=json&$filter=externalCode like '%PSN%'`;

            $.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let oEventReasonsModel = new JSONModel(data.d.results);
                    that.getView().setModel(oEventReasonsModel, "eventReasonsModel");
                },
                error: function () {
                    MessageToast.show("Failed to fetch event reasons.");
                }
            });
        },

        formatExternalCodeWithName: function (externalCode, name) {
            return name + "  -  " + externalCode;
        },

        formatExternalCodeWithCurrentStatus: function (externalCode, CurrentStatus) {
            return CurrentStatus + "  -  " + externalCode;
        },

        formatExternalCodeWithItem: function (externalCode, item) {
            return item + "  -  " + externalCode;
        },

        formatExternalCodeWithExternalName: function (externalCode, externalName_en_US) {
            return externalName_en_US + "  -  " + externalCode;
        },

        formatExternalCodeWithJobTitle: function (externalCode, jobTitle) {
            return jobTitle + "  -  " + externalCode;
        },

        formatRecordStatus: function (value) {
            if (!value) {
                return "REQUEST COMPLETED";
            } else if (value === "PENDING") {
                return "PENDING";
            } else if (value === "COMPLETED") {
                return "POSITION SUBMITTED";

            } else if (value === "REQUEST COMPLETED") {
                return "REQUEST COMPLETED";
            }
            return value;
        },

        formatRecordStatusState: function (value) {
            if (value === "COMPLETED") {
                return sap.ui.core.ValueState.Success;
            } else if (value === "PENDING") {
                return sap.ui.core.ValueState.Error;
            } else if (!value || value === "REQUEST COMPLETED") {
                return sap.ui.core.ValueState.Warning;
            }
            return sap.ui.core.ValueState.None;
        },



        formatMaritalStatus: function (value) {
            return value === true ? "Married" : "Unmarried";
        },

        formatDate: function (value) {
            if (value) {
                let timestamp = parseInt(value.replace("/Date(", "").replace(")/", ""), 10);
                let hireDate = new Date(timestamp);
                let oDateFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({ pattern: "yyyy-MM-dd" });
                return oDateFormat.format(hireDate);
            }
            return value;
        },

        formatYear: function (value) {
            if (value) {
                let timestamp = parseInt(value.replace("/Date(", "").replace(")/", ""), 10);
                let hireDate = new Date(timestamp);
                return hireDate.getFullYear();
            }
            return value;
        },





        formatTenureDate: function (value) {
            if (value) {
                let timestamp = parseInt(value.replace("/Date(", "").replace(")/", ""), 10);
                let hireDate = new Date(timestamp);
                let oDateFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({ pattern: "yyyy-MM-dd" });

                let currentDate = new Date();

                let timeDiff = currentDate - hireDate;

                let diffInYears = Math.floor(timeDiff / (1000 * 60 * 60 * 24 * 365.25));
                return diffInYears + " years";
            }
            return value;
        },

        formatAttachmentLink: function (fileName) {
            return fileName || "";
        },

        serialNumberFormat: function (oContext, aData) {
            if (!this._serialNumberMap) {
                this._serialNumberMap = new WeakMap();
            }

            if (!this._serialNumberMap.has(oContext)) {
                let index = aData.indexOf(oContext) + 1;
                this._serialNumberMap.set(oContext, index);
            }

            return this._serialNumberMap.get(oContext);
        },



        onAttachmentLinkPress: function (oEvent) {
            const oContext = oEvent.getSource().getBindingContext("ListData");
            const attachmentId = oContext.getProperty("cust_AttachmentNav/attachmentId");
            if (attachmentId) {
                // Show loading indicator
                BusyIndicator.show(0);

                // Fetch the attachment data based on attachmentId
                this.fetchAttachmentData(attachmentId)
                    .then(attachmentData => {
                        // Display the attachment data in a dialog
                        this.showAttachmentDialog(attachmentData);
                        BusyIndicator.hide();
                    })
                    .catch(error => {
                        // Handle errors
                        BusyIndicator.hide();
                        MessageToast.show("Error fetching attachment data: " + error.message);
                    });
            } else {
                MessageToast.show("No attachment ID found");
            }
        },

        showAttachmentDialog: function (attachmentData) {
            let oView = this.getView();

            // Load the fragment only once
            if (!this._oAttachmentDialog) {
                Fragment.load({
                    name: "com.taqa.psnform.taqapsnform.view.Attachment",
                    controller: this
                }).then(function (oDialog) {
                    this._oAttachmentDialog = oDialog;
                    oView.addDependent(this._oAttachmentDialog);

                    // Set up the dialog content with attachment data
                    this._setupAttachmentContent(attachmentData);

                    this._oAttachmentDialog.open();
                }.bind(this));
            } else {
                // Update dialog content with attachment data
                this._setupAttachmentContent(attachmentData);
                this._oAttachmentDialog.open();
            }
        },


        // Function to fetch attachment data
        fetchAttachmentData: function (attachmentId) {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: this.getPath("SF_1") + `/Attachment(${attachmentId}L)?$format=json`,
                    method: "GET",
                    headers: {
                        "Accept": "application/json"
                    },
                    success: function (data) {
                        if (data && data.d) {
                            resolve(data.d); // Resolve with JSON data
                        } else {
                            reject(new Error("Invalid data structure received"));
                        }
                    },
                    error: function () {
                        // Fallback to XML if JSON fails
                        $.ajax({
                            url: this.getPath("SF_1") + `/Attachment(${attachmentId}L)`,
                            method: "GET",
                            dataType: "xml",
                            success: function (xmlData) {
                                let $xml = $(xmlData);

                                // Extract data from XML
                                const attachmentData = {
                                    attachmentId: $xml.find("d\\:attachmentId, attachmentId").text(),
                                    fileName: $xml.find("d\\:fileName, fileName").text(),
                                    mimeType: $xml.find("d\\:mimeType, mimeType").text(),
                                    fileContent: $xml.find("d\\:fileContent, fileContent").text(), // Extract file content
                                    fileSize: $xml.find("d\\:fileSize, fileSize").text(),
                                    fileExtension: $xml.find("d\\:fileExtension, fileExtension").text()
                                };

                                resolve(attachmentData); // Resolve with XML data
                            },
                            error: function (xmlError) {
                                reject(xmlError); // Reject if both JSON and XML fail
                            }
                        });
                    }
                });
            });
        },



        // Dynamic Image Base64 to Iamge

        _setupAttachmentContent: function (attachmentData) {
            // Get controls from fragment
            let oTitle = sap.ui.getCore().byId("attachmentDialogTitle") || this.byId("attachmentDialogTitle");
            let oTextArea = sap.ui.getCore().byId("attachmentTextArea") || this.byId("attachmentTextArea");
            let oImage = sap.ui.getCore().byId("attachmentImage") || this.byId("attachmentImage");

            // Set dialog title with file name
            if (oTitle) {
                oTitle.setText(attachmentData.fileName || "Attachment");
            }

            // Display file information in TextArea
            if (oTextArea) {
                oTextArea.setValue("File Information:\n" +
                    "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
                    "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
                    "Size: " + (attachmentData.fileSize || "Unknown") + " bytes");
                oTextArea.setVisible(true);
            }

            // Determine file type based on mimeType
            const isImage = ["image/jpeg", "image/png", "image/gif", "image/svg+xml"].includes(attachmentData.mimeType);
            const isPdf = attachmentData.mimeType === "application/pdf";
            const isDocument = ["application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation"]
                .includes(attachmentData.mimeType);

            if (isImage && oImage) {
                // Display image preview for image files
                oImage.setSrc("data:" + attachmentData.mimeType + ";base64," + attachmentData.fileContent);
                oImage.setWidth("100%");
                oImage.setHeight("auto");
                oImage.setAlt(attachmentData.fileName || "Attachment");
                oImage.setVisible(true);

                // Hide the TextArea since we are displaying an image
                oTextArea.setVisible(false);
            } else if (isPdf && oTextArea) {
                // For PDF files, provide a link to view the file
                oTextArea.setValue("File Information:\n" +
                    "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
                    "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
                    "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
                    `<a href="data:${attachmentData.mimeType};base64,${attachmentData.fileContent}" target="_blank">View PDF</a>`);

                // Hide the Image control
                oImage.setVisible(false);
            } else if (isDocument && oTextArea) {
                // For document files, provide a download link
                oTextArea.setValue("File Information:\n" +
                    "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
                    "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
                    "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
                    `<a href="data:${attachmentData.mimeType};base64,${attachmentData.fileContent}" download="${attachmentData.fileName}">Download Document</a>`);

                // Hide the Image control
                oImage.setVisible(false);
            } else if (oTextArea) {
                // Display a placeholder message for unsupported file types
                oTextArea.setValue("File Information:\n" +
                    "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
                    "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
                    "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
                    "Preview not available for this file.");

                // Hide the Image control
                oImage.setVisible(false);
            }
        },


        // // Static Data
        // _setupAttachmentContent: function (attachmentData) {

        //     let oTitle = sap.ui.getCore().byId("attachmentDialogTitle") || this.byId("attachmentDialogTitle");
        //     let oTextArea = sap.ui.getCore().byId("attachmentTextArea") || this.byId("attachmentTextArea");
        //     let oImage = sap.ui.getCore().byId("attachmentImage") || this.byId("attachmentImage");

        //     let fileContent1 = "";

        //     if (oTitle) {
        //         oTitle.setText(attachmentData.fileName || "Attachment");
        //     }

        //     if (oTextArea) {
        //         oTextArea.setValue("File Information:\n" +
        //             "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
        //             "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
        //             "Size: " + (attachmentData.fileSize || "Unknown") + " bytes");
        //         oTextArea.setVisible(true);
        //     }

        //     const isImage = ["image/jpeg", "image/png", "image/gif", "image/svg+xml"].includes(attachmentData.mimeType);
        //     const isPdf = attachmentData.mimeType === "application/pdf";
        //     const isDocument = ["application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        //         "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        //         "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation"]
        //         .includes(attachmentData.mimeType);

        //     if (isImage && oImage) {
        //         oImage.setSrc("data:" + attachmentData.mimeType + ";base64," + fileContent1); // Use static fileContent1
        //         oImage.setWidth("100%");
        //         oImage.setHeight("auto");
        //         oImage.setAlt(attachmentData.fileName || "Attachment");
        //         oImage.setVisible(true);
        //         oTextArea.setVisible(false);
        //     } else if (isPdf && oTextArea) {
        //         oTextArea.setValue("File Information:\n" +
        //             "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
        //             "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
        //             "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
        //             `<a href="data:${attachmentData.mimeType};base64,${fileContent1}" target="_blank">View PDF</a>`); // Use static fileContent1

        //         // Hide the Image control
        //         oImage.setVisible(false);
        //     } else if (isDocument && oTextArea) {
        //         // For document files, provide a download link
        //         oTextArea.setValue("File Information:\n" +
        //             "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
        //             "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
        //             "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
        //             `<a href="data:${attachmentData.mimeType};base64,${fileContent1}" download="${attachmentData.fileName}">Download Document</a>`); // Use static fileContent1

        //         // Hide the Image control
        //         oImage.setVisible(false);
        //     } else if (oTextArea) {
        //         // Display a placeholder message for unsupported file types
        //         oTextArea.setValue("File Information:\n" +
        //             "Name: " + (attachmentData.fileName || "Unknown") + "\n" +
        //             "Type: " + (attachmentData.mimeType || "Unknown") + "\n" +
        //             "Size: " + (attachmentData.fileSize || "Unknown") + " bytes\n\n" +
        //             "Preview not available for this file.");

        //         oImage.setVisible(false);
        //     }
        // },


        onDialogClose: function () {
            if (this._oAttachmentDialog) {
                this._oAttachmentDialog.close();
            }
        },

        onDownloadAttachment: function () {
            let oModel = this.getView().getModel("AttachmentData");
            if (!oModel) {
                return;
            }

            let oData = oModel.getData();
            let sFileContent = oData.fileContent;
            let sMimeType = oData.mimeType;
            let sFileName = oData.fileName;

            let byteCharacters = atob(sFileContent);
            let byteArrays = [];

            for (let i = 0; i < byteCharacters.length; i++) {
                byteArrays.push(byteCharacters.charCodeAt(i));
            }

            let blob = new Blob([new Uint8Array(byteArrays)], { type: sMimeType });
            let url = URL.createObjectURL(blob);

            let a = document.createElement("a");
            document.body.appendChild(a);
            a.style = "display: none";
            a.href = url;
            a.download = sFileName;
            a.click();

            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
        },

        onDialogClose: function () {
            if (this._oAttachmentDialog) {
                this._oAttachmentDialog.close();
            }
        },

        getSelectedRowData: function () {
            return this.oSelectedRowModel.getProperty("/selectedRow");
        },

        handleFullScreen: function () {
            this.oFlexibleColumnLayout.setLayout(fioriLibrary.LayoutType.MidColumnFullScreen);
            this.oViewModel.setProperty("/showExitFullScreen", true);
            this.oViewModel.setProperty("/showFullScreen", false);
        },

        handleExitFullScreen: function () {
            this.oFlexibleColumnLayout.setLayout(fioriLibrary.LayoutType.TwoColumnsMidExpanded);
            this.oViewModel.setProperty("/showExitFullScreen", false);
            this.oViewModel.setProperty("/showFullScreen", true);
        },

        handleClose: function () {
            this.oFlexibleColumnLayout.setLayout(fioriLibrary.LayoutType.OneColumn);
            this.oViewModel.setProperty("/showExitFullScreen", false);
            this.oViewModel.setProperty("/showFullScreen", true);

            this.oViewModel.setProperty("/showSearchSort", false);
            this.oViewModel.setProperty("/showRaiseRequest", true);
        },

        onLayoutChange: function (oEvent) {
            let sLayout = oEvent.getParameter("layout");
            let isFullScreen = sLayout === fioriLibrary.LayoutType.MidColumnFullScreen;

            let isExpanded = sLayout === fioriLibrary.LayoutType.OneColumn;
            let isCollapsed = sLayout === fioriLibrary.LayoutType.TwoColumnsMidExpanded ||
                sLayout === fioriLibrary.LayoutType.MidColumnFullScreen;

            this.oViewModel.setProperty("/showSearchSort", isCollapsed);
            this.oViewModel.setProperty("/showRaiseRequest", isExpanded);

            this.oViewModel.setProperty("/showExitFullScreen", isFullScreen);
            this.oViewModel.setProperty("/showFullScreen", !isFullScreen);
        },

        onSearchIconPress: function () {
            let oSearchField = this.byId("idSearchField");
            let oSearchButton = this.byId("idSearchButton");

            oSearchField.setVisible(true);
            oSearchButton.setVisible(false);
        },

        onOpenChangeApprovalDialog: function () {
            let oView = this.getView();

            if (!this._oRequestDialog) {

                this._oRequestDialog = sap.ui.xmlfragment(
                    oView.getId(),
                    "com.taqa.psnform.taqapsnform.view.PSNRequest",
                    this
                );

                oView.addDependent(this._oRequestDialog);

                if (!this.oEmployeeSearchModel) {
                    this.oEmployeeSearchModel = new sap.ui.model.json.JSONModel();
                }

                this.oEmployeeSearchModel.setData({
                    employees: []
                });

                this._oRequestDialog.open();
            } else {
                this._oRequestDialog.open();
            }
        },

        onEmployeeSearch: function (oEvent) {
            let sValue = "";
            if (oEvent.getParameter("suggestValue") !== undefined) {
                sValue = oEvent.getParameter("suggestValue");
                console.log("Suggest value:", sValue);
            } else if (oEvent.getParameter("query") !== undefined) {
                sValue = oEvent.getParameter("query");
                console.log("Query value:", sValue);
            }
            this._searchTerm = sValue;

            if (sValue && sValue.length >= 2) {
                this._searchEmployees(sValue);
            } else {
                this.oEmployeeSearchModel.setProperty("/employees", []);
            }
        },


        _searchEmployees: function (sSearchTerm) {
            console.log("Searching for employees with term:", sSearchTerm);

            let sSearchLower = sSearchTerm.toLowerCase();
            let sFilter = "tolower(username) like '%" + sSearchLower + "%' or " +
                "tolower(firstName) like '%" + sSearchLower + "%' or " +
                "tolower(lastName) like '%" + sSearchLower + "%' or " +
                "tolower(userId) like '%" + sSearchLower + "%'";
            let sPath = this.getPath("SF_1");

            // Construct the full URL with query parameters
            let sUrl = sPath + "/User";

            // Make the AJAX call
            jQuery.ajax({
                url: sUrl,
                method: "GET",
                headers: {
                    "Accept": "application/json"
                },
                data: {
                    "$top": "10",
                    "$filter": sFilter
                },
                success: function (data) {
                    console.log("Search results:", data);
                    // if (data && data.results)
                    if (data && data.d && data.d.results) {
                        let aEmployees = data.d.results.map(function (emp) {
                            return {
                                userId: emp.userId || "",
                                firstName: emp.firstName || "",
                                lastName: emp.lastName || "",
                                username: emp.username || "",
                                hireDate: emp.hireDate || "",
                                employeeType: emp.empInfo && emp.empInfo.label ? emp.empInfo.label : "N/A",
                                displayName: (emp.firstName || "") + " " + (emp.lastName || "")
                            };
                        });
                        this.oEmployeeSearchModel.setProperty("/employees", aEmployees);
                        console.log("Updated employee suggestions:", aEmployees);
                    }
                }.bind(this),
                error: function (jqXHR, textStatus, errorThrown) {
                    console.error("Error fetching employee data:", textStatus, errorThrown);
                    this.oEmployeeSearchModel.setProperty("/employees", []);
                }.bind(this)
            });
        },

        onSuggestionItemSelected: function (oEvent) {
            console.log("Suggestion item selected event:", oEvent);
            let oItem = oEvent.getParameter("listItem");

            if (oItem) {

                let oCustomData = oItem.getCustomData().find(function (data) {
                    return data.getKey() === "userId";
                });

                let sEmployeeId = oCustomData ? oCustomData.getValue() : "";
                this._selectedEmployeeId = sEmployeeId;

                let sEmployeeName = oItem.getTitle();

                let oSearchField = this.byId("idEmployeeSearch");
                if (oSearchField) {
                    oSearchField.setValue(sEmployeeName);

                    this.byId("idEmployeeList").setVisible(false);
                }

                console.log("Selected employee ID:", sEmployeeId, "Name:", sEmployeeName);
            }
        },



        onReject: function () {
            if (!this._oRejectDialog) {
                this._oRejectDialog = sap.ui.xmlfragment(
                    "com.taqa.psnform.taqapsnform.view.Reject", // Replace with your fragment path
                    this
                );
                this.getView().addDependent(this._oRejectDialog);
            }

            this._oRejectDialog.open();
        },


        onRejectRequest: function () {

            let wfRequestId = this._currentWfRequestId || "defaultRequestId";

            this._rejectWfRequestWithComment(wfRequestId);
            this._oRejectDialog.close();

        },

        onCloseRejectDialog: function () {
            if (this._oRejectDialog) {
                this._oRejectDialog.close();
            }
        },

        onAccept: function () {
            MessageBox.confirm("Are you sure you want to approve?", {
                actions: [MessageBox.Action.YES, MessageBox.Action.NO],
                onClose: function (sAction) {
                    if (sAction === MessageBox.Action.YES) {
                        this.onApproveWfRequest();
                        MessageToast.show("Approval Request Submitted!");

                    }
                }.bind(this)
            });
        },

        onReturn: function () {
            MessageBox.confirm("Are you sure you want to return?", {
                title: "Confirm Return",
                actions: [MessageBox.Action.YES, MessageBox.Action.NO],
                onClose: function (sAction) {
                    if (sAction === MessageBox.Action.YES) {
                        this.onSendbackWfRequest();
                        console.log("User confirmed Return.");
                        MessageToast.show("Returning...");
                        // Add your Return logic here
                    }
                }.bind(this) // Bind the controller's 'this' context
            });
        },

        onWithdraw: function () {
            MessageBox.confirm("Are you sure you want to withdraw?", {
                title: "Confirm Withdraw",
                actions: [MessageBox.Action.YES, MessageBox.Action.NO],
                onClose: function (sAction) {
                    if (sAction === MessageBox.Action.YES) {
                        this.onWithdrawWfRequest();
                        console.log("User confirmed Withdraw.");
                        MessageToast.show("Withdrawing...");
                    }
                }.bind(this) // Bind the controller's 'this' context
            });
        },

        _loadTypeofChangePicklist: function () {
            let oModel = this.getOwnerComponent().getModel();

            if (!oModel) {
                console.error("OData model not found");
                return;
            }

            let sPath = "/Picklist('TypeofChange')/picklistOptions";

            oModel.read(sPath, {
                urlParameters: {
                    "$expand": "picklistLabels",
                    "$select": "externalCode,id,picklistLabels/id,picklistLabels/optionId,picklistLabels/label,picklistLabels/locale",
                    "$filter": "picklistLabels/locale eq 'en_US'"
                },
                success: function (oData) {
                    console.log("Full OData Response:", oData);

                    if (oData && oData.results) {
                        let aEventReasons = oData.results.map(function (item) {
                            let oLabel = item.picklistLabels.results.find(function (label) {
                                return label.locale === "en_US";
                            });

                            return {
                                externalCode: item.externalCode,
                                name: oLabel ? oLabel.label : item.externalCode
                            };
                        });
                        aEventReasons.sort((a, b) => a.name.localeCompare(b.name));
                        this.oEventReasonsModel.setProperty("/eventReasons", aEventReasons);
                        console.log("Loaded required actions:", aEventReasons);
                    } else {
                        console.warn("No results found in picklist options");
                    }
                }.bind(this),
                error: function (oError) {
                    console.error("Error fetching required actions:", oError);
                    if (oError.responseText) {
                        try {
                            let errorDetails = JSON.parse(oError.responseText);
                            console.error("Detailed Error:", errorDetails);
                        } catch (e) {
                            console.error("Error parsing error response");
                        }
                    }
                }.bind(this)
            });
        },


        onCloseDialog: function () {
            if (this._oRequestDialog) {
                let oSearchField = this.byId("idEmployeeSearch");
                if (oSearchField) {
                    oSearchField.setValue("");
                }

                this.oEmployeeSearchModel.setProperty("/employees", []);

                // Clear selected employee
                this._selectedEmployeeId = null;

                this._oRequestDialog.close();
            }
        },

        onChangeApprover: function () {
            if (!this._selectedItemContext) {
                MessageBox.error("Please select an item from the list.");
                return;
            }

            let wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");
            let wfRequestStepId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestStepNav/results/0/wfRequestStepId");
            let userId = this._selectedEmployeeId;

            if (!wfRequestId || !wfRequestStepId || !userId) {
                MessageBox.error("Please provide all required fields.");
                return;
            }

            $.ajax({
                url: this.getPath("SF_OAUTH") + `/changeWfRequestApprover?wfRequestId=${wfRequestId}L&wfRequestStepId=${wfRequestStepId}L&updateToUserId='${userId}'&editTransaction='NO_EDIT'`,
                type: "POST",
                success: function () {
                    MessageToast.show("Approver changed successfully.");
                    this._getPendingListDetails();
                }.bind(this),
                error: function (jqXHR, textStatus, errorThrown) {
                    let errorMessage = "Error changing approver: " + errorThrown;

                    // Parse error response (JSON or XML)
                    if (jqXHR.responseJSON && jqXHR.responseJSON.error && jqXHR.responseJSON.error.message && jqXHR.responseJSON.error.message.value) {
                        errorMessage = "Error changing approver: " + jqXHR.responseJSON.error.message.value;
                    } else if (jqXHR.responseXML) {
                        try {
                            const xmlDoc = jqXHR.responseXML;
                            const messageElement = xmlDoc.querySelector("message");
                            if (messageElement) {
                                errorMessage = "Error changing approver: " + messageElement.textContent;
                            }
                        } catch (xmlParseError) {
                            console.error("Error parsing XML response:", xmlParseError);
                        }
                    } else if (jqXHR.responseText) {
                        errorMessage = "Error changing approver: " + jqXHR.responseText;
                    }

                    MessageBox.error(errorMessage, {
                        title: "Error",
                        details: jqXHR.responseText
                    });
                    console.error("Error changing approver:", textStatus, errorThrown, jqXHR);
                }.bind(this)
            });
            this.onCloseDialog();
        },

        // UnAuthorize OAUTH destination function

        // onApproveWfRequest: function () {
        //     let wfRequestId;

        //     if (!this._selectedItemContext) {
        //         let oWfDataModel = this.getView().getModel("wfData_0");
        //         wfRequestId = oWfDataModel.getProperty("/wfRequestId");

        //         if (!wfRequestId) {
        //             sap.m.MessageBox.error("Please select an item from the list or ensure WF Request ID is available in the form.");
        //             return;
        //         }
        //     } else {
        //         const status = this._selectedItemContext.getProperty("wfRequestNav/results/0/status");

        //         if (status === "PENDING") {
        //             wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");

        //             if (!wfRequestId) {
        //                 sap.m.MessageBox.error("WF Request ID is missing for the selected pending item.");
        //                 return;
        //             }

        //         } else {
        //             let oWfDataModel = this.getView().getModel("wfData_0");
        //             wfRequestId = oWfDataModel.getProperty("/wfRequestId");

        //             if (!wfRequestId) {
        //                 sap.m.MessageBox.error("WF Request ID is missing from the form data for this item.");
        //                 return;
        //             }
        //         }
        //     }

        //     $.ajax({
        //         url: this.getPath("SF_OAUTH") + `/approveWfRequest?wfRequestId=${wfRequestId}`,
        //         type: "POST",
        //         success: function (data, textStatus, jqXHR) {
        //             sap.m.MessageToast.show("Workflow request approved successfully.");
        //             this._updateButtonVisibilityAfterApproval();
        //             this._getWorkflowDetails(this._currentUserId);
        //         }.bind(this),
        //         error: function (jqXHR, textStatus, errorThrown) {
        //             let errorMessage = "Error approving workflow request: " + errorThrown;

        //             if (jqXHR.responseJSON && jqXHR.responseJSON.error?.message?.value) {
        //                 errorMessage = "Error approving workflow request: " + jqXHR.responseJSON.error.message.value;
        //             } else if (jqXHR.responseXML) {
        //                 try {
        //                     const xmlDoc = jqXHR.responseXML;
        //                     const messageElement = xmlDoc.querySelector("message");
        //                     if (messageElement) {
        //                         errorMessage = "Error approving workflow request: " + messageElement.textContent;
        //                     }
        //                 } catch (xmlParseError) {
        //                     console.error("Error parsing XML response:", xmlParseError);
        //                 }
        //             } else if (jqXHR.responseText) {
        //                 errorMessage = "Error approving workflow request: " + jqXHR.responseText;
        //             }

        //             sap.m.MessageBox.error(errorMessage, {
        //                 title: "Error",
        //                 details: jqXHR.responseText
        //             });
        //             console.error("Error approving workflow request:", textStatus, errorThrown, jqXHR);
        //         }.bind(this)
        //     });
        // },



        onApproveWfRequest: function () {
            let wfRequestId;
            let sfPath;
            let requestType;
            let userId = this._currentUserId;
            BusyIndicator.show(0);

            try {
                if (!this._selectedItemContext) {
                    let oWfDataModel = this.getView().getModel("wfData_0");

                    if (!oWfDataModel) {
                        BusyIndicator.hide();
                        MessageBox.error("No workflow data available. Please select an item or refresh the view.");
                        return;
                    }

                    wfRequestId = oWfDataModel.getProperty("/wfRequestId");
                    requestType = "SubWfRequestId";

                    if (!wfRequestId) {
                        BusyIndicator.hide();
                        MessageBox.error("Please select an item from the list or ensure WF Request ID is available in the form.");
                        return;
                    }

                    const currentStepNum = oWfDataModel.getProperty("/currentStepNum");
                    sfPath = this._determineSFPathByStep(currentStepNum);
                } else {
                    const status = this._selectedItemContext.getProperty("wfRequestNav/results/0/status");

                    if (status === "PENDING") {
                        wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");
                        requestType = "ReqWfRequestId";

                        if (!wfRequestId) {
                            BusyIndicator.hide();
                            MessageBox.error("WF Request ID is missing for the selected pending item.");
                            return;
                        }

                        const currentStepNum = this._selectedItemContext.getProperty("wfRequestNav/results/0/currentStepNum");
                        sfPath = this._determineSFPathByStep(currentStepNum);
                    } else {
                        let oWfDataModel = this.getView().getModel("wfData_0");

                        if (!oWfDataModel) {
                            BusyIndicator.hide();
                            MessageBox.error("No workflow data available for this item.");
                            return;
                        }

                        wfRequestId = oWfDataModel.getProperty("/wfRequestId");
                        requestType = "SubWfRequestId";

                        if (!wfRequestId) {
                            BusyIndicator.hide();
                            MessageBox.error("WF Request ID is missing from the form data for this item.");
                            return;
                        }

                        const currentStepNum = oWfDataModel.getProperty("/currentStepNum");
                        sfPath = this._determineSFPathByStep(currentStepNum);
                    }
                }

                // Log approval attempt
                console.log("Attempting to approve workflow:", {
                    wfRequestId: wfRequestId,
                    path: sfPath,
                    requestType: requestType,
                    userId: userId
                });

                // Construct the approval URL
                const url = this.getPath(sfPath) + `/approveWfRequest?wfRequestId=${wfRequestId}L`;

                // AJAX call to approve the workflow
                $.ajax({
                    url: url,
                    type: "POST",
                    dataType: "xml",
                    success: function (data) {
                        try {
                            // Parse XML response
                            const statusElement = $(data).find("d\\:status, status");
                            const wfRequestIdElement = $(data).find("d\\:wfRequestId, wfRequestId");

                            if (statusElement.length > 0 && statusElement.text() === "success") {
                                const responseWfRequestId = wfRequestIdElement.length > 0 ? wfRequestIdElement.text() : wfRequestId;

                                // Show success message and then reload
                                MessageBox.success(`Workflow request ${responseWfRequestId} approved successfully.`, {
                                    onClose: function () {
                                        window.location.reload();
                                    }
                                });
                            } else {
                                BusyIndicator.hide();
                                MessageBox.warning("Approval response received but status was not successful.", {
                                    title: "Warning"
                                });
                            }
                        } catch (parseError) {
                            console.error("Error parsing success response:", parseError);
                            BusyIndicator.hide();

                            // Assume success if parsing fails but call was successful and reload
                            MessageToast.show("Workflow request approved successfully.");
                            window.location.reload();
                        }
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        BusyIndicator.hide();
                        let errorMessage = "Error approving workflow request: " + errorThrown;

                        if (jqXHR.responseJSON && jqXHR.responseJSON.error?.message?.value) {
                            errorMessage = "Error approving workflow request: " + jqXHR.responseJSON.error.message.value;
                        } else if (jqXHR.responseXML) {
                            try {
                                const xmlDoc = jqXHR.responseXML;
                                const messageElement = xmlDoc.querySelector("message");
                                if (messageElement) {
                                    errorMessage = "Error approving workflow request: " + messageElement.textContent;
                                }
                            } catch (xmlParseError) {
                                console.error("Error parsing XML response:", xmlParseError);
                            }
                        } else if (jqXHR.responseText) {
                            try {
                                const responseObj = JSON.parse(jqXHR.responseText);
                                if (responseObj.error && responseObj.error.message) {
                                    errorMessage = "Error: " + responseObj.error.message;
                                }
                            } catch (e) {
                                errorMessage = "Error approving workflow request: " + jqXHR.responseText;
                            }
                        }

                        MessageBox.error(errorMessage, {
                            title: "Error",
                            details: jqXHR.responseText
                        });
                        console.error("Error approving workflow request:", textStatus, errorThrown, jqXHR);
                    }
                });
            } catch (ex) {
                BusyIndicator.hide();
                MessageBox.error("An unexpected error occurred: " + ex.message);
                console.error("Exception in onApproveWfRequest:", ex);
            }
        },

        _determineSFPathByStep: function (stepNum) {
            if (stepNum === 1) {
                return "SF_3";
            } else if (stepNum === 2) {
                return "SF_4";
            } else if (stepNum === 3) {
                return "SF_5";
            } else {
                return "SF_OAUTH";
            }
        },


        _checkWorkflowCompletionStatus: function (wfRequestId, callback) {
            // First, get the current workflow status
            let sDetailUrl = this.getPath("SF_1") +
                `/WfRequest(${wfRequestId}L)?$format=json&$select=wfRequestId,totalSteps,currentStepNum,status`;

            $.ajax({
                url: sDetailUrl,
                type: "GET",
                dataType: "json",
                success: function (oDetailData) {
                    if (oDetailData && oDetailData.d) {
                        let status = oDetailData.d.status;
                        let currentStepNum = oDetailData.d.currentStepNum;
                        let totalSteps = oDetailData.d.totalSteps;
                        let isComplete = (status === "COMPLETED" || (currentStepNum >= totalSteps));

                        console.log("Workflow status check:", {
                            wfRequestId: wfRequestId,
                            status: status,
                            currentStep: currentStepNum,
                            totalSteps: totalSteps,
                            isComplete: isComplete
                        });

                        callback(isComplete, status);  // Pass the status back to the callback
                    } else {
                        console.error("Invalid response format for workflow status check");
                        callback(false, "");
                    }
                },
                error: function (xhr, status, error) {
                    console.error("Error checking workflow completion status:", error);
                    MessageToast.show("Error checking workflow status: " + error);
                    callback(false, "");
                }
            });
        },

        _refreshApplicationData: function (wfRequestId, requestType, userId) {
            let that = this;

            // Keep busy indicator showing during refresh
            BusyIndicator.show(0);

            // Check workflow completion status
            this._checkWorkflowCompletionStatus(wfRequestId, function (isComplete, status) {
                if (isComplete) {
                    if (requestType === "ReqWfRequestId") {
                        // For request-based approvals
                        that._updateButtonVisibilityAfterApproval();
                        that._getWorkflowDetails(userId);
                    } else {
                        // For submission-based approvals
                        if (status === "") {
                            // Don't modify the status if it's already COMPLETED
                            // Let it naturally progress to REQUEST COMPLETED through other APIs
                            that._updateButtonVisibilityAfterApproval();
                            that._getWorkflowDetails(userId);
                        } else {
                            // Only update status if not already completed
                            that._updatePositionStatus(wfRequestId, "REQUEST COMPLETED", function () {
                                that._updateButtonVisibilityAfterApproval();
                                that._getWorkflowDetails(userId);
                            });
                        }
                    }
                } else {
                    // Workflow not complete
                    that._updateButtonVisibilityAfterApproval();
                    that._getWorkflowDetails(userId);
                }

                // Hide busy indicator when all operations are complete
                setTimeout(function () {
                    BusyIndicator.hide();
                }, 500); // Small delay to ensure UI updates are visible
            });
        },

        _updatePositionStatus: function (wfRequestId, status, callback) {
            let userId = this._currentUserId;

            if (!userId) {
                console.error("User ID not available for updating position status");
                MessageToast.show("Error: User ID not available");
                return;
            }
            let oDataModel = this.getView().getModel("DataModel");
            let aChangeOfStatus = oDataModel.getProperty("/ChangeOfStatus");

            // Find the row for "Job Title"
            let oJobTitleRow = aChangeOfStatus.find(function (row) {
                return row.Item === "Job Title";
            });

            let sSelectedJobTitle = oJobTitleRow ? oJobTitleRow.NewStatus : "";
            let sEffectiveDate = this._currentEffectiveDate || new Date().toISOString().split('T')[0];

            // Create the payload for updating position status
            let oPayloadData = {
                "__metadata": {
                    "uri": "PositionUpdate"
                },
                "userId": userId,
                "wfRequestId": wfRequestId,
                "jobTitle": sSelectedJobTitle,
                "effectiveDate": sEffectiveDate,
                "status": status
            };

            let sUrl = this.getPath("SF_6") + "/updatePositionStatus";

            $.ajax({
                url: sUrl,
                type: "POST",
                contentType: "application/json",
                data: JSON.stringify(oPayloadData),
                success: function (data) {
                    console.log("Position status updated successfully:", data);
                    MessageToast.show("Position status updated to: " + status);

                    if (typeof callback === "function") {
                        callback();
                    }
                },
                error: function (xhr, textStatus, errorThrown) {
                    let errorMessage = "Error updating position status: " + errorThrown;

                    if (xhr.responseJSON && xhr.responseJSON.error) {
                        errorMessage = "Error: " + xhr.responseJSON.error.message.value;
                    }

                    console.error("Error updating position status:", errorMessage);
                    MessageBox.error(errorMessage);
                }
            });
        },

        _updateButtonVisibilityAfterApproval: function () {
            if (this.byId("approveButton")) {
                this.byId("approveButton").setVisible(true);

            }
            this._refreshView();
        },



        _refreshView: function () {
            let oButtonModel = this.getOwnerComponent().getModel("buttonModel") || this.oButtonModel;

            if (oButtonModel) {
                oButtonModel.setProperty("/showApproveButton", true);
                oButtonModel.setProperty("/showSubmitButton", false);
                oButtonModel.setProperty("/showCompletedStatus", true);
            }
            let oView = this.getView();
            if (oView) {
                let oWorkflowTable = oView.byId("workflowStatusTable");
                if (oWorkflowTable) {
                    oWorkflowTable.getBinding("items").refresh(true);
                }
            }
        },

        highlightSubmitApprovalsSection: function () {
            let oApprovalSection = this.byId("submitApprovalsSection");

            if (oApprovalSection) {
                oApprovalSection.focus();
                oApprovalSection.addStyleClass("highlightedSection");
                setTimeout(function () {
                    oApprovalSection.removeStyleClass("highlightedSection");
                }, 1500);
            }
        },



        onSendbackWfRequest: function () {
            let wfRequestId;

            if (!this._selectedItemContext) {
                let oWfDataModel = this.getView().getModel("wfData_0");
                wfRequestId = oWfDataModel.getProperty("/wfRequestId");

                if (!wfRequestId) {
                    MessageBox.error("Please select an item from the list or ensure WF Request ID is available in the form.");
                    return;
                }
            } else {
                const status = this._selectedItemContext.getProperty("wfRequestNav/results/0/status");

                if (status === "PENDING") {
                    wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");

                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing for the selected pending item.");
                        return;
                    }

                } else {
                    let oWfDataModel = this.getView().getModel("wfData_0");
                    wfRequestId = oWfDataModel.getProperty("/wfRequestId");

                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing from the form data for this item.");
                        return;
                    }
                }
            }

            $.ajax({
                url: this.getPath("SF_OAUTH") + `/sendbackWfRequest?wfRequestId=${wfRequestId}`,
                type: "POST",
                success: function () {
                    MessageToast.show("Workflow request sent back successfully.");
                    this._getPendingListDetails();
                }.bind(this),
                error: function (jqXHR, textStatus, errorThrown) {
                    let errorMessage = "Error sending back workflow request: " + errorThrown;

                    if (jqXHR.responseJSON && jqXHR.responseJSON.error?.message?.value) {
                        errorMessage = "Error sending back workflow request: " + jqXHR.responseJSON.error.message.value;
                    } else if (jqXHR.responseXML) {
                        try {
                            const xmlDoc = jqXHR.responseXML;
                            const messageElement = xmlDoc.querySelector("message");
                            if (messageElement) {
                                errorMessage = "Error sending back workflow request: " + messageElement.textContent;
                            }
                        } catch (xmlParseError) {
                            console.error("Error parsing XML response:", xmlParseError);
                        }
                    } else if (jqXHR.responseText) {
                        errorMessage = "Error sending back workflow request: " + jqXHR.responseText;
                    }

                    MessageBox.error(errorMessage, {
                        title: "Error",
                        details: jqXHR.responseText
                    });
                    console.error("Error sending back workflow request:", textStatus, errorThrown, jqXHR);
                }.bind(this)
            });
        },





        onWithdrawWfRequest: function () {
            let wfRequestId;

            if (!this._selectedItemContext) {
                const oWfDataModel = this.getView().getModel("wfData_0");
                wfRequestId = oWfDataModel.getProperty("/wfRequestId") + "L";

                if (!wfRequestId) {
                    MessageBox.error("Please select an item from the list or ensure WF Request ID is available in the form.");
                    return;
                }
            } else {
                const status = this._selectedItemContext.getProperty("wfRequestNav/results/0/status");

                if (status === "PENDING") {
                    wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");
                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing for the selected pending item.");
                        return;
                    }
                } else {
                    const oWfDataModel = this.getView().getModel("wfData_0");
                    wfRequestId = oWfDataModel.getProperty("/wfRequestId") + "L";

                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing from the form data for this item.");
                        return;
                    }
                }
            }

            $.ajax({
                url: this.getPath("SF_OAUTH") + `/withdrawWfRequest?wfRequestId=${wfRequestId}`,
                type: "POST",
                success: function () {
                    MessageToast.show("Workflow request withdrawn successfully.");
                    this._getPendingListDetails();
                }.bind(this),
                error: function (jqXHR, textStatus, errorThrown) {
                    let errorMessage = "Error withdrawing workflow request: " + errorThrown;
                    if (jqXHR.responseJSON?.error?.message?.value) {
                        errorMessage = "Error withdrawing workflow request: " + jqXHR.responseJSON.error.message.value;
                    } else if (jqXHR.responseXML) {
                        try {
                            const xmlDoc = jqXHR.responseXML;
                            const messageElement = xmlDoc.querySelector("message");
                            if (messageElement) {
                                errorMessage = "Error withdrawing workflow request: " + messageElement.textContent;
                            }
                        } catch (xmlParseError) {
                            console.error("Error parsing XML response:", xmlParseError);
                        }
                    } else if (jqXHR.responseText) {
                        errorMessage = "Error withdrawing workflow request: " + jqXHR.responseText;
                    }

                    MessageBox.error(errorMessage, {
                        title: "Error",
                        details: jqXHR.responseText
                    });
                    console.error("Error withdrawing workflow request:", textStatus, errorThrown, jqXHR);
                }.bind(this)
            });
        },



        onRejectWfRequest: function () {
            let wfRequestId;

            if (!this._selectedItemContext) {
                let oWfDataModel = this.getView().getModel("wfData_0");
                wfRequestId = oWfDataModel.getProperty("/wfRequestId");

                if (!wfRequestId) {
                    MessageBox.error("Please select an item from the list or ensure WF Request ID is available in the form.");
                    return;
                }
            } else {
                const status = this._selectedItemContext.getProperty("wfRequestNav/results/0/status");

                if (status === "PENDING") {
                    wfRequestId = this._selectedItemContext.getProperty("wfRequestNav/results/0/wfRequestId");

                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing for the selected pending item.");
                        return;
                    }

                } else {
                    let oWfDataModel = this.getView().getModel("wfData_0");
                    wfRequestId = oWfDataModel.getProperty("/wfRequestId");

                    if (!wfRequestId) {
                        MessageBox.error("WF Request ID is missing from the form data for this item.");
                        return;
                    }
                }
            }

            this._showRejectDialog(wfRequestId);
        },


        _showRejectDialog: function (wfRequestId) {
            let that = this;
            Fragment.load({
                name: "com.taqa.psnform.taqapsnform.view.Reject",
                controller: this,
                id: "rejectFragment"
            }).then(function (oDialog) {
                that._oRejectDialog = oDialog;
                that.getView().addDependent(that._oRejectDialog);
                that._oRejectDialog.open();
                that._wfRequestIdForRejection = wfRequestId; // Store wfRequestId
            });
        },


        onRejectRequest: function () {
            let oReasonTextArea = Fragment.byId("rejectFragment", "idRejectReason");

            if (oReasonTextArea) {
                let sReason = oReasonTextArea.getValue();
                if (sReason) {
                    this._rejectWfRequestWithComment(this._wfRequestIdForRejection, sReason);
                } else {
                    MessageToast.show("Please enter a rejection reason.");
                }
            } else {
                MessageToast.show("Error: Rejection reason input not found.");
            }
        },


        onCloseRejectDialog: function () {
            if (this._oRejectDialog) {
                this._oRejectDialog.close();
                this._oRejectDialog.destroy();
                this._oRejectDialog = null;
                this._wfRequestIdForRejection = null;
            }
        },


        _rejectWfRequestWithComment: function (wfRequestId, comment) {
            if (!comment) {
                MessageToast.show("Please enter a rejection reason.");
                return;
            }

            BusyIndicator.show();

            let url = this.getPath() + `/commentWfRequest?wfRequestId=${wfRequestId}L&comment='${encodeURIComponent(comment)}'`;

            $.ajax({
                url: url,
                type: "POST",
                success: function () {
                    BusyIndicator.hide();
                    MessageToast.show("Workflow request rejected successfully.");
                    this._getPendingListDetails();
                    this.onCloseRejectDialog();
                }.bind(this),
                error: function (jqXHR, textStatus, errorThrown) {
                    BusyIndicator.hide();

                    let errorMessage = "Error rejecting workflow request: " + errorThrown;

                    if (jqXHR.responseJSON && jqXHR.responseJSON.error?.message?.value) {
                        errorMessage = "Error rejecting workflow request: " + jqXHR.responseJSON.error.message.value;
                    } else if (jqXHR.responseXML) {
                        try {
                            const xmlDoc = jqXHR.responseXML;
                            const messageElement = xmlDoc.querySelector("message");
                            if (messageElement) {
                                errorMessage = "Error rejecting workflow request: " + messageElement.textContent;
                            }
                        } catch (xmlParseError) {
                            console.error("Error parsing XML response:", xmlParseError);
                        }
                    } else if (jqXHR.responseText) {
                        errorMessage = "Error rejecting workflow request: " + jqXHR.responseText;
                    }

                    MessageBox.error(errorMessage, {
                        title: "Error",
                        details: jqXHR.responseText
                    });
                    console.error("Error rejecting workflow request:", textStatus, errorThrown, jqXHR);
                    this.onCloseRejectDialog();
                }.bind(this)
            });
        },


        fetchEmpJobData: function (userId) {
            if (!userId) {
                console.error("No userId provided for fetchEmpJobData");
                return;
            }

            let sServiceUrl = this.getPath("SF_1") + `/EmpJob?$format=JSON&$filter=userId eq '${userId}'&$expand=companyNav,departmentNav,divisionNav,locationNav,workscheduleCodeNav,managerUserNav,costCenterNav,positionNav&$select=companyNav/externalCode,companyNav/name_en_US,departmentNav/externalCode,departmentNav/name_en_US,divisionNav/externalCode,divisionNav/name_en_US,locationNav/externalCode,locationNav/name,workscheduleCodeNav/externalCode,workscheduleCodeNav/externalName_en_US,jobCode,jobTitle,managerUserNav/userId,managerUserNav/displayName,costCenterNav/name,positionNav/externalName_en_US`;
            let that = this;

            jQuery.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let empJobData = data.d.results[0];

                    if (empJobData) {
                        that.empJobData = empJobData;
                        // Store fetched data
                        console.log("Successfully fetched employee job data for user:", userId);
                    } else {
                        MessageToast.show("No employee job data found for user: " + userId);
                        console.warn("No employee job data found for user:", userId);
                    }
                },
                error: function (e) {
                    MessageToast.show("Error fetching employee job data");
                    console.error("Error fetching employee job data for user:", userId, e);
                }
            });
        },


        fetchCompanyData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/FOCompany?$format=JSON&$select=externalCode,name";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {

                    let data1 = data.d.results
                    console.log(data1)


                    let companyModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(companyModel, "CompanyModel");


                },
                error: function (e) {
                    MessageToast.show("Error fetching company data");
                    console.error("Error fetching company data:", e);
                }
            });
        },

        fetchDepartmentData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/FODepartment?$format=JSON&$select=externalCode,name";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let departmentModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(departmentModel, "DepartmentModel");
                },
                error: function (e) {
                    MessageToast.show("Error fetching department data");
                    console.error("Error fetching department data:", e);
                }
            });
        },

        fetchDivisionData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/FODivision?$format=JSON&$select=externalCode,name";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let divisionModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(divisionModel, "DivisionModel");
                },
                error: function (e) {
                    MessageToast.show("Error fetching division data");
                    console.error("Error fetching division data:", e);
                }
            });
        },

        fetchLocationData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/FOLocation?$format=JSON&$select=externalCode,name,locationGroup";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let locationModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(locationModel, "LocationModel");
                },
                error: function (e) {
                    MessageToast.show("Error fetching location data");
                    console.error("Error fetching location data:", e);
                }
            });
        },

        fetchWorkScheduleData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/WorkSchedule?$format=JSON&$select=externalCode,externalName_en_US";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let workScheduleModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(workScheduleModel, "WorkScheduleModel");
                },
                error: function (e) {
                    MessageToast.show("Error fetching work schedule data");
                    console.error("Error fetching work schedule data:", e);
                }
            });
        },

        fetchPositionData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/Position?$format=JSON&$select=code,externalName_en_US,jobCode,jobTitle";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let positionModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(positionModel, "PositionModel");
                  
                },
                error: function (e) {
                    MessageToast.show("Error fetching position data");
                    console.error("Error fetching position data:", e);
                }
            });
        },

        fetchCostCenterData: function () {
            let that = this;
            let url = this.getPath("SF_1") + "/FOCostCenter?$format=json&&$select=externalCode,name";

            jQuery.ajax({
                url: url,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let CostCenterModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(CostCenterModel, "CostCenterModel");
                    
                   
                },
                error: function (e) {
                    MessageToast.show("Error fetching position data");
                    console.error("Error fetching position data:", e);
                }
            });
        },




        fetchEmpCompensationData: function (userId) {
            let that = this;

            let sServiceUrl = this.getPath("SF_1") + `/EmpCompensation?$format=JSON&$filter=userId eq '${userId}'&$expand=empPayCompRecurringNav/payComponentNav,payGroupNav,employmentNav/jobInfoNav/payGradeNav,&$select=empPayCompRecurringNav/payComponent,empPayCompRecurringNav/paycompvalue,empPayCompRecurringNav/currencyCode,empPayCompRecurringNav/frequency,empPayCompRecurringNav/payComponentNav/externalCode,empPayCompRecurringNav/payComponentNav/name,payGroupNav/externalCode,payGroupNav/name,employmentNav/jobInfoNav/payGradeNav/externalCode,employmentNav/jobInfoNav/payGradeNav/name`;

            jQuery.ajax({

                url: sServiceUrl,

                type: "GET",

                dataType: "json",

                async: true,

                success: function (data) {

                    let empCompensationModel = new sap.ui.model.json.JSONModel(data.d);

                    that.getView().setModel(empCompensationModel, "EmpCompensationModel");

                    console.log("EmpCompensation data fetched successfully:", data.d);
                    that.fetchGrossMonthlySalaryData(userId);

                    that.fetchPayGradeData(); // Fetch pay grade data for the combobox

                },

                error: function (e) {

                    MessageToast.show("Error fetching employee compensation data.");

                    console.error("Error fetching employee compensation data:", e);

                }

            });

        },

        fetchGrossMonthlySalaryData: function (userId) {
            let that = this;
            let sGrossSalaryServiceUrl = this.getPath("SF_1") + `/EmpCompensation?$format=JSON&$filter=userId eq '${userId}'&$expand=empPayCompRecurringNav&$select=empPayCompRecurringNav/paycompvalue,empPayCompRecurringNav/currencyCode,empPayCompRecurringNav/frequency`;

            jQuery.ajax({
                url: sGrossSalaryServiceUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let totalMonthlySalary = 0;
                    let currencyCode = "";

                    if (data && data.d && data.d.results && data.d.results.length > 0 &&
                        data.d.results[0].empPayCompRecurringNav &&
                        data.d.results[0].empPayCompRecurringNav.results) {
                        data.d.results[0].empPayCompRecurringNav.results.forEach(function (payComponent) {
                            if (payComponent.frequency === "MON" && payComponent.paycompvalue) {
                                totalMonthlySalary += parseFloat(payComponent.paycompvalue);
                                currencyCode = payComponent.currencyCode;
                            } else if (payComponent.frequency !== "MON" && payComponent.paycompvalue) {
                                console.warn("Pay component with frequency:", payComponent.frequency, "not considered for total monthly salary.");
                            }
                        });
                    }

                    let grossSalaryData = {
                        GrossMonthlySalary: totalMonthlySalary,
                        Currency: currencyCode
                    };

                    let grossSalaryModel = new sap.ui.model.json.JSONModel(grossSalaryData);
                    that.getView().setModel(grossSalaryModel, "GrossSalaryModel");
                    console.log("Gross Monthly Salary data fetched and calculated (separate call):", grossSalaryData);
                },
                error: function (e) {
                    MessageToast.show("Error fetching gross monthly salary data.");
                    console.error("Error fetching gross monthly salary data:", e);
                }
            });
        },

        fetchPayGradeData: function () {
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/FOPayGrade?$format=JSON&$select=name,externalCode";

            jQuery.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let payGradeModel = new sap.ui.model.json.JSONModel(data.d.results);
                    that.getView().setModel(payGradeModel, "PayGradeModel");
                    that.initializeChangeOfCompensationTable(that.getView().getModel("EmpCompensationModel").getProperty("/results/0"));
                    that.initializeGrossCompensationTable(data.d.results[0]);
                },
                error: function (e) {
                    MessageToast.show("Error fetching Pay Grade data.");
                    console.error("Error fetching Pay Grade data:", e);
                }
            });
        },

        initializeChangeOfCompensationTable: function (empCompensationData) {
            let changeOfCompensation = [];

            changeOfCompensation.push({
                SNo: "1",
                Item: "Grade",
                externalCode: empCompensationData && empCompensationData.employmentNav &&
                    empCompensationData.employmentNav.jobInfoNav &&
                    empCompensationData.employmentNav.jobInfoNav.results[0] &&
                    empCompensationData.employmentNav.jobInfoNav.results[0].payGradeNav ?
                    empCompensationData.employmentNav.jobInfoNav.results[0].payGradeNav.externalCode : "",
                CurrentValue: empCompensationData && empCompensationData.employmentNav && empCompensationData.employmentNav.jobInfoNav && empCompensationData.employmentNav.jobInfoNav.results[0] && empCompensationData.employmentNav.jobInfoNav.results[0].payGradeNav ?
                    empCompensationData.employmentNav.jobInfoNav.results[0].payGradeNav.externalCode + " - " + empCompensationData.employmentNav.jobInfoNav.results[0].payGradeNav.name : "",
                NewValue: ""
            });

            if (empCompensationData && empCompensationData.empPayCompRecurringNav && empCompensationData.empPayCompRecurringNav.results) {
                empCompensationData.empPayCompRecurringNav.results.forEach(function (payComponent, index) {
                    changeOfCompensation.push({
                        SNo: (index + 2).toString(), // Start SNo from 2
                        Item: payComponent.payComponentNav ? payComponent.payComponentNav.name : "",
                        externalCode: payComponent.payComponentNav ? payComponent.payComponentNav.externalCode : "",
                        CurrentValue: payComponent.paycompvalue || "",
                        NewValue: ""
                    });
                });
            }

            let dataModel = new sap.ui.model.json.JSONModel({ ChangeOfCompensation: changeOfCompensation });
            this.getView().setModel(dataModel, "ChangeOfCompensationModel");
            this.bindChangeOfCompensationTable();
            this.getView().byId("changeOfCompSection").setVisible(true);
        },


        bindChangeOfCompensationTable: function () {
            let that = this;
            let table = this.getView().byId("compensationTable");

            table.bindItems({
                path: "ChangeOfCompensationModel>/ChangeOfCompensation",
                factory: function (sId, oContext) {
                    let item = oContext.getObject();
                    let cells = [
                        new Text({ text: "{ChangeOfCompensationModel>SNo}" }),
                        new Text({
                            text: {
                                parts: [
                                    { path: "ChangeOfCompensationModel>Item" },
                                    { path: "ChangeOfCompensationModel>externalCode" }
                                ],
                                formatter: that.formatExternalCodeWithItem
                            }
                        }),
                        new Text({
                            text: {
                                parts: [
                                    { path: "ChangeOfCompensationModel>SNo" },
                                    { path: "ChangeOfCompensationModel>CurrentValue" }
                                ],
                                formatter: function (sno, value) {
                                    if (sno !== "1" && value) {
                                        return value + "  SAR";
                                    }
                                    return value || "";
                                }
                            }
                        })
                    ];

                    let newValueControl;

                    if (item.SNo === "1") {
                        newValueControl = new ComboBox({
                            selectedKey: "{ChangeOfCompensationModel>NewValue}",
                            items: {
                                path: "PayGradeModel>/",
                                template: new sap.ui.core.Item({
                                    key: "{PayGradeModel>name}",
                                    text: {
                                        parts: [
                                            { path: "PayGradeModel>name" },
                                            { path: "PayGradeModel>externalCode" }
                                        ],
                                        formatter: that.formatExternalCodeWithName
                                    }
                                })
                            },
                            change: function (oEvent) {
                                let selectedItem = oEvent.getParameter("selectedItem");
                                if (selectedItem) {
                                    let bindingContext = oEvent.getSource().getBindingContext("ChangeOfCompensationModel");
                                    let path = bindingContext.getPath();
                                    let model = oEvent.getSource().getModel("ChangeOfCompensationModel");
                                    model.setProperty(path + "/NewValue", selectedItem.getKey());
                                }
                            }
                        });
                    } else {
                        newValueControl = new Input({
                            value: "{ChangeOfCompensationModel>NewValue}",
                            type: sap.m.InputType.Number,
                            placeholder: "Enter new value"
                        });
                    }

                    cells.push(newValueControl);

                    return new ColumnListItem({
                        cells: cells
                    });
                }
            });
        },

        initializeGrossCompensationTable: function (empCompensationData) {
            let contractType = empCompensationData && empCompensationData.employmentNav ?
                empCompensationData.employmentNav.contractType : "";

            let grossCompensationData = [
                {
                    SNo: "9",
                    Item: "Contract Type",
                    CurrentValue: contractType,
                    NewValue: "",
                    isContractType: true
                },
                {
                    SNo: "10",
                    Item: "Leave Entitlement",
                    CurrentValue: "As Per the Policy",
                    NewValue: "As Per the Policy"
                },
                {
                    SNo: "11",
                    Item: "Medical Insurance Class",
                    CurrentValue: "A+ Class",
                    NewValue: "As Per the Policy"
                },
                {
                    SNo: "12",
                    Item: "Employee Ticket Entitlement",
                    CurrentValue: "As Per the Policy",
                    NewValue: "As Per the Policy"
                },
                {
                    SNo: "13",
                    Item: "Family Ticket Entitlement",
                    CurrentValue: "As Per the Policy",
                    NewValue: "As Per the Policy"
                }
            ];

            let grossCompensationModel = new sap.ui.model.json.JSONModel(grossCompensationData);
            this.getView().setModel(grossCompensationModel, "GrossCompensationModel");
            this.bindGrossCompensationTable();
        },

        bindGrossCompensationTable: function () {
            let table = this.getView().byId("grosscompensationTable");

            table.bindItems({
                path: "GrossCompensationModel>/",
                template: new ColumnListItem({
                    cells: [
                        new Text({ text: "{GrossCompensationModel>SNo}" }),
                        new Text({ text: "{GrossCompensationModel>Item}" }),
                        // Current Value
                        new VBox({
                            items: [
                                new Input({
                                    value: "{GrossCompensationModel>CurrentValue}",
                                    visible: "{= ${GrossCompensationModel>isContractType} === true }",
                                    placeholder: "Enter current value"
                                }),
                                new Text({
                                    text: "{GrossCompensationModel>CurrentValue}",
                                    visible: "{= ${GrossCompensationModel>isContractType} !== true }"
                                })
                            ]
                        }),
                        // New Value
                        new VBox({
                            items: [
                                new Select({
                                    selectedKey: "{GrossCompensationModel>NewValue}",
                                    visible: "{= ${GrossCompensationModel>isContractType} === true }",
                                    width: "100%",
                                    items: [
                                        new sap.ui.core.Item({ key: "Single Status", text: "Single Status" }),
                                        new sap.ui.core.Item({ key: "Family Status", text: "Family Status" })
                                    ]
                                }),
                                new Text({
                                    text: "As Per the Policy",
                                    visible: "{= ${GrossCompensationModel>isContractType} !== true }"
                                })
                            ]
                        })
                    ]
                })
            });
        },






        _getUserIdAndOpenAddComponentDialog: function () {
            let userId = null;
            let cust_EffectiveDate = null;
            let psnTypeChange = null; // New variable to store cust_PSNTypeChange
            let that = this;

            return new Promise(function (resolve, reject) {
                let oList = that.byId("idList");
                let oSelectedItem = oList.getSelectedItem();

                if (!oSelectedItem) {
                    MessageToast.show("No record selected. Please select a record first.");
                    console.error("No record selected for opening Add Component dialog");
                    reject(new Error("No record selected"));
                    return;
                }

                let oSelectedContext = oSelectedItem.getBindingContext("ListData");

                if (!oSelectedContext) {
                    MessageToast.show("Invalid selection. Cannot retrieve data.");
                    console.error("Invalid selection context for opening Add Component dialog");
                    reject(new Error("Invalid selection context"));
                    return;
                }

                let oSelectedData = oSelectedContext.getObject();

                userId = oSelectedData.externalCode;
                psnTypeChange = oSelectedData.cust_PSNTypeChange; // Fetch the cust_PSNTypeChange value

                if (oSelectedData.cust_EffectiveDate) {
                    let timestamp = parseInt(oSelectedData.cust_EffectiveDate.replace("/Date(", "").replace(")/", ""));
                    let date = new Date(timestamp);
                    cust_EffectiveDate = date.getFullYear() + "-" +
                        String(date.getMonth() + 1).padStart(2, '0') + "-" +
                        String(date.getDate()).padStart(2, '0');
                }

                console.log("Retrieved from selected list item - userId:", userId, "cust_EffectiveDate:", cust_EffectiveDate, "psnTypeChange:", psnTypeChange);

                if (!userId || !cust_EffectiveDate || !psnTypeChange) {
                    MessageToast.show("Could not retrieve user ID, effective date, or PSN Type Change from selection.");
                    console.error("Could not retrieve user ID, effective date, or PSN Type Change from selection");
                    reject(new Error("Could not retrieve user data"));
                    return;
                }

                that._currentUserId = userId;
                that._currentEffectiveDate = cust_EffectiveDate;
                that._currentPsnTypeChange = psnTypeChange; // Store the fetched value

                resolve();
            });
        },

        onAddComponent: function () {
            let oView = this.getView();
            let that = this;

            this._getUserIdAndOpenAddComponentDialog();
            //     .then(function () {
            if (!that._pDialog) {
                that._pDialog = Fragment.load({
                    id: oView.getId(),
                    name: "com.taqa.psnform.taqapsnform.view.AddComponent",
                    controller: that
                }).then(function (oDialog) {
                    oView.addDependent(oDialog);
                    return oDialog;
                });
            }
            that._pDialog.then(function (oDialog) {
                oDialog.open();
                that.fetchAddComponentData();
            });
            //     })
            //     .catch(function (error) {
            //         console.error("Error during _getUserIdAndOpenAddComponentDialog:", error);
            //     });
        },




        fetchAddComponentData: function () {
            let that = this;
            let sServiceUrl = this.getPath("SF_1") + "/FOPayComponent?$format=JSON&$select=name,payComponentType,externalCode,frequencyCode";

            jQuery.ajax({
                url: sServiceUrl,
                type: "GET",
                dataType: "json",
                async: true,
                success: function (data) {
                    let addComponentModel = new JSONModel(data.d);
                    that.getView().setModel(addComponentModel, "AddComponentModel");

                    console.log("Fetched Data: ", addComponentModel.getData());

                },
                error: function (e) {
                    MessageToast.show("Error fetching Pay Component data.");
                    console.error("Error fetching Pay Component data:", e);
                }
            });
        },


        onSubmitAddComponent: function () {
            let that = this;
            let oDialog = this.byId("addComponentDialog");
            let oPayComponentSelect = this.byId("payComponentSelect");
            let oValueInput = this.byId("valueInput");

            let sSelectedComponentCode = oPayComponentSelect.getSelectedKey();

            let sValue = oValueInput.getValue();

            if (!sSelectedComponentCode) {
                MessageToast.show("Please select a pay component");
                return;
            }

            if (!sValue) {
                MessageToast.show("Please enter a value");
                return;
            }

            this._getUserIdAndOpenAddComponentDialog()
                .then(function () {
                    let sUserId = that._currentUserId;
                    let sEffectiveDate = that._currentEffectiveDate;

                    console.log("Submitting with userId:", sUserId, "effectiveDate:", sEffectiveDate);

                    let oPayloadData = {
                        userId: sUserId,
                        effectiveDate: sEffectiveDate,
                        componentCode: sSelectedComponentCode,
                        value: sValue
                    };

                    let oBusyDialog = new sap.m.BusyDialog();
                    oBusyDialog.open();

                    let sUrl = that.getPath("SF_1") + "/upsert";

                    $.ajax({
                        url: sUrl,
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify(oPayloadData),
                        success: function () {
                            oBusyDialog.close();
                            MessageToast.show("Pay component added successfully");
                            oPayComponentSelect.setSelectedKey("");
                            oValueInput.setValue("");
                            oDialog.close();

                            that._refreshPayComponentsList();
                        },
                        error: function (jqXHR, textStatus, errorThrown) {
                            oBusyDialog.close();

                            let sErrorMessage = "Failed to add pay component";
                            try {
                                let oErrorResponse = JSON.parse(jqXHR.responseText);
                                if (oErrorResponse && oErrorResponse.message) {
                                    sErrorMessage = oErrorResponse.message || sErrorMessage;
                                }
                            } catch (e) {
                                if (errorThrown) {
                                    sErrorMessage += ": " + errorThrown;
                                }
                            }

                            MessageBox.error(sErrorMessage);
                        }
                    });
                })
                .catch(function (error) {
                    MessageBox.error("Could not submit: " + error.message);
                });
        },



        onCancelAddComponent: function () {
            if (this._pDialog) {
                this._pDialog.then(function (oDialog) {
                    oDialog.close();
                });
            }
        },


        // initializeChangeOfStatusTable: function () {
        //     let empJobData = this.empJobData; // Assuming this is set elsewhere
        //     let companyNames = this.companyNames;
        //     console.log(companyNames);

        //     // Get the button model
        //     var buttonModel = this.getView().getModel("buttonModel");
        //     // var changeOfStatusVisible = buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/changeOfStatus");

        //     let changeOfStatus = [
        //         {
        //             SNo: "1",
        //             Item: "Company",
        //             CurrentStatus: empJobData.companyNav.externalCode + " - " + empJobData.companyNav.name_en_US,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableCompany") || false
        //         },
        //         {
        //             SNo: "2",
        //             Item: "Cost Center",
        //             CurrentStatus: empJobData.costCenterNav.externalCode + " - " + empJobData.costCenterNav.name,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableCostCenter") || false
        //         },
        //         {
        //             SNo: "3",
        //             Item: "Position",
        //             CurrentStatus: empJobData.positionNav.code + "  -  " + empJobData.positionNav.externalName_en_US,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enablePosition") || false
        //         },
        //         {
        //             SNo: "4",
        //             Item: "Department",
        //             CurrentStatus: empJobData.departmentNav.externalCode + "  -  " + empJobData.departmentNav.name_en_US,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableDepartment") || false
        //         },
        //         {
        //             SNo: "5",
        //             Item: "Division",
        //             CurrentStatus: empJobData.divisionNav.externalCode + "  -  " + empJobData.divisionNav.name_en_US,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableDivision") || false
        //         },
        //         {
        //             SNo: "6",
        //             Item: "Location",
        //             CurrentStatus: empJobData.locationNav.externalCode + "  -  " + empJobData.locationNav.name,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableLocation") || false
        //         },
        //         {
        //             SNo: "7",
        //             Item: "Work Schedule",
        //             CurrentStatus: empJobData.workscheduleCodeNav.externalCode + "  -  " + empJobData.workscheduleCodeNav.externalName_en_US,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableWorkSchedule") || false
        //         },
        //         {
        //             SNo: "8",
        //             Item: "Manager",
        //             CurrentStatus: empJobData.managerUserNav.userId + "  -  " + empJobData.managerUserNav.displayName,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableManager") || false
        //         },
        //         {
        //             SNo: "9",
        //             Item: "Job Title",
        //             CurrentStatus: empJobData.jobCode + "  -  " + empJobData.jobTitle,
        //             NewStatus: "",
        //             Visible: buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/enableJobTitle") || false
        //         }
        //     ];

        //     // Filter the array to only include items where the bound property is true
        //     let visibleItems = changeOfStatus.filter(function (item) {
        //         // This ensures we're using the dynamically bound Visible property value
        //         return item.Visible === true;
        //     });

        //     console.log(visibleItems);

        //     let dataModel = new sap.ui.model.json.JSONModel({ ChangeOfStatus: visibleItems });
        //     this.getView().setModel(dataModel, "DataModel");

        //     this.bindTableItems();

        //     // Make section visible based on the property from buttonModel
        //     let changeOfStatusSection = this.getView().byId("changeOfStatusSection");
        //     if (changeOfStatusSection) {
        //         changeOfStatusSection.setVisible(true);
        //     }
        // },


 
initializeChangeOfStatusTable: function () {
    let empJobData = this.empJobData;
    let companyNames = this.companyNames;
    console.log(companyNames);

    // Get the flat button model for visibility
    let buttonModel = this.getView().getModel("buttonModel");
    let visibilityConfig = buttonModel.getData();

    let changeOfStatus = [
        {
            SNo: "1",
            Item: "Company",
            CurrentStatus: empJobData.companyNav.externalCode + " - " + empJobData.companyNav.name_en_US,
            NewStatus: "",
            Visible: visibilityConfig.enableCompany || false
        },
        {
            SNo: "2",
            Item: "Cost Center",
            CurrentStatus: empJobData.costCenterNav.externalCode + " - " + empJobData.costCenterNav.name,
            NewStatus: "",
            Visible: visibilityConfig.enableCostCenter || false
        },
        {
            SNo: "3",
            Item: "Position",
            CurrentStatus: empJobData.positionNav.code + " - " + empJobData.positionNav.externalName_en_US,
            NewStatus: "",
            Visible: visibilityConfig.enablePosition || false
        },
        {
            SNo: "4",
            Item: "Department",
            CurrentStatus: empJobData.departmentNav.externalCode + " - " + empJobData.departmentNav.name_en_US,
            NewStatus: "",
            Visible: visibilityConfig.enableDepartment || false
        },
        {
            SNo: "5",
            Item: "Division",
            CurrentStatus: empJobData.divisionNav.externalCode + " - " + empJobData.divisionNav.name_en_US,
            NewStatus: "",
            Visible: visibilityConfig.enableDivision || false
        },
        {
            SNo: "6",
            Item: "Location",
            CurrentStatus: empJobData.locationNav.externalCode + " - " + empJobData.locationNav.name,
            NewStatus: "",
            Visible: visibilityConfig.enableLocation || false
        },
        {
            SNo: "7",
            Item: "Work Schedule",
            CurrentStatus: empJobData.workscheduleCodeNav.externalCode + " - " + empJobData.workscheduleCodeNav.externalName_en_US,
            NewStatus: "",
            Visible: visibilityConfig.enableWorkSchedule || false
        },
        {
            SNo: "8",
            Item: "Manager",
            CurrentStatus: empJobData.managerUserNav.userId + " - " + empJobData.managerUserNav.displayName,
            NewStatus: "",
            Visible: visibilityConfig.enableManager || false
        },
        {
            SNo: "9",
            Item: "Job Title",
            CurrentStatus: empJobData.jobCode + " - " + empJobData.jobTitle,
            NewStatus: "",
            Visible: visibilityConfig.enableJobTitle || false
        }
    ];

    // Filter only visible rows
    let visibleItems = changeOfStatus.filter(item => item.Visible === true);
    console.log(visibleItems);

    // Set filtered data model
    let dataModel = new sap.ui.model.json.JSONModel({ ChangeOfStatus: visibleItems });
    this.getView().setModel(dataModel, "DataModel");

    this.bindTableItems();

    // Show the section if any items are visible
    let changeOfStatusSection = this.getView().byId("changeOfStatusSection");
    if (changeOfStatusSection) {
        changeOfStatusSection.setVisible(visibleItems.length > 0);
    }
},



        bindTableItems: function () {
            let that = this;
            let table = this.getView().byId("changeStatusTable");
            let buttonModel = this.getView().getModel("buttonModel");

            table.bindItems({
                path: "DataModel>/ChangeOfStatus",
                factory: function (sId, oContext) {
                    let item = oContext.getObject();

                    let cells = [
                        new sap.m.Text({ text: "{DataModel>SNo}" }),
                        new sap.m.Text({ text: "{DataModel>Item}" }),
                        new sap.m.Text({ text: "{DataModel>CurrentStatus}" })
                    ];

                    // Extract the current status code - assuming format is "code - name"
                    let currentStatusParts = item.CurrentStatus.split(" - ");
                    let currentStatusCode = currentStatusParts[0].trim();


                    // Initialize ComboBox with appropriate configuration based on item type
                    let comboBox;

                    let enable = buttonModel.getProperty("/ButtonVisibility/RequestTypes/PSN_JobTitleChange/Country HR/editable");


                    switch (item.SNo) {

                        case "1": // Company
                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Company",
                                enabled: enable,
                                items: {
                                    path: "CompanyModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{CompanyModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "CompanyModel>externalCode" },
                                                { path: "CompanyModel>name" }
                                            ],
                                            formatter: that.formatExternalCodeWithName
                                        }
                                    })
                                }
                            });
                            break;

                        case "2": // Cost Center
                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Cost Center",
                                enabled: enable,
                                items: {
                                    path: "CostCenterModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{CostCenterModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "CostCenterModel>externalCode" },
                                                { path: "CostCenterModel>name" }
                                            ],
                                            formatter: that.formatExternalCodeWithName
                                        }
                                    })
                                }
                            });
                            break;

                        case "3": // Position
                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Position",
                                enabled: enable,
                                items: {
                                    path: "PositionModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{PositionModel>code}",
                                        text: {
                                            parts: [
                                                { path: "PositionModel>code" },
                                                { path: "PositionModel>externalName_en_US" }
                                            ],
                                            formatter: that.formatExternalCodeWithExternalName
                                        }
                                    })
                                }
                            });
                            break;

                        case "4": // Department

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Department",
                                enabled: enable,
                                items: {
                                    path: "DepartmentModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{DepartmentModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "DepartmentModel>externalCode" },
                                                { path: "DepartmentModel>name" }
                                            ],
                                            formatter: that.formatExternalCodeWithName
                                        }
                                    })
                                }
                            });
                            break;

                        case "5": // Division

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Division",
                                enabled: enable,
                                items: {
                                    path: "DivisionModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{DivisionModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "DivisionModel>externalCode" },
                                                { path: "DivisionModel>name" }
                                            ],
                                            formatter: that.formatExternalCodeWithName
                                        }
                                    })
                                }
                            });
                            break;

                        case "6": // Location

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Location",
                                enabled: enable,
                                items: {
                                    path: "LocationModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{LocationModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "LocationModel>externalCode" },
                                                { path: "LocationModel>name" }
                                            ],
                                            formatter: that.formatExternalCodeWithName
                                        }
                                    })
                                }
                            });
                            break;

                        case "7": // Work Schedule

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Work Schedule",
                                enabled: enable,
                                items: {
                                    path: "WorkScheduleModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{WorkScheduleModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "WorkScheduleModel>externalCode" },
                                                { path: "WorkScheduleModel>externalName_en_US" }
                                            ],
                                            formatter: that.formatExternalCodeWithExternalName
                                        }
                                    })
                                }
                            });
                            break;

                        case "8": // Manager

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Manager",
                                enabled: enable,
                                items: {
                                    path: "PositionModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{PositionModel>code}",
                                        text: {
                                            parts: [
                                                { path: "PositionModel>externalName_en_US" },
                                                { path: "PositionModel>code" }
                                            ],
                                            formatter: that.formatExternalCodeWithExternalName
                                        }
                                    })
                                }
                            });
                            break;

                        case "9": // Job Title

                            comboBox = new sap.m.ComboBox({
                                selectedKey: currentStatusCode,
                                placeholder: "Select Job Title",
                                enabled: enable,
                                items: {
                                    path: "PositionModel>/",
                                    template: new sap.ui.core.Item({
                                        key: "{PositionModel>externalCode}",
                                        text: {
                                            parts: [
                                                { path: "PositionModel>externalCode" },
                                                { path: "PositionModel>jobTitle" }
                                            ],
                                            formatter: that.formatExternalCodeWithJobTitle
                                        }
                                    })
                                }
                            });
                            break;

                        default:
                            comboBox = new sap.m.Input({
                                value: item.CurrentStatus
                            });
                    }

                    // Attach change event to track selection changes
                    comboBox.attachSelectionChange(function (oEvent) {
                        let selectedItem = oEvent.getParameter("selectedItem");
                        if (selectedItem) {
                            let bindingContext = oEvent.getSource().getBindingContext("DataModel");
                            let path = bindingContext.getPath();
                            let model = that.getView().getModel("DataModel");

                            // Get the text representation of the selected item
                            let selectedText = selectedItem.getText();

                            // Set the NewStatus with the newly selected item's key and text
                            model.setProperty(path + "/NewStatus", selectedItem.getKey() + " - " + selectedText);
                        }
                    });

                    cells.push(comboBox);

                    return new sap.m.ColumnListItem({
                        cells: cells
                    });
                }
            });
        },






        // bindTableItems: function () {
        //     let that = this;
        //     let table = this.getView().byId("changeStatusTable");

        //     // Store the original data for later reference
        //     let changeOfStatusData = this.getView().getModel("DataModel").getProperty("/ChangeOfStatus");
        //     console.log("Original Change of Status Data:", JSON.stringify(changeOfStatusData));

        //     // First, update the table binding
        //     table.bindItems({
        //         path: "DataModel>/ChangeOfStatus",
        //         factory: function (sId, oContext) {
        //             let item = oContext.getObject();
        //             let rowPath = oContext.getPath();

        //             // Create the basic text cells
        //             let cells = [
        //                 new sap.m.Text({ text: "{DataModel>SNo}" }),
        //                 new sap.m.Text({ text: "{DataModel>Item}" }),
        //                 new sap.m.Text({ text: "{DataModel>CurrentStatus}" })
        //             ];

        //             // Create the ComboBox for the last cell
        //             let comboBox = createComboBox(item);
        //             cells.push(comboBox);

        //             return new sap.m.ColumnListItem({
        //                 cells: cells
        //             });
        //         }
        //     });

        //     // Function to create the appropriate ComboBox for each item
        //     function createComboBox(item) {
        //         // Get current status value to preselect
        //         let currentStatusValue = item.NewStatus || item.CurrentStatus;
        //         console.log(`Item ${item.SNo}: Will try to preselect "${currentStatusValue}"`);

        //         let comboBox;
        //         let modelName, keyPath, textPath, formatter;

        //         // Configure model and path based on item type
        //         switch (item.SNo) {
        //             case "1": // Company
        //                 modelName = "CompanyModel";
        //                 keyPath = "externalCode";
        //                 textPath = "name";
        //                 formatter = that.formatExternalCodeWithName;
        //                 break;

        //             case "2": // Cost Center
        //                 modelName = "CostCenterModel";
        //                 keyPath = "externalCode";
        //                 textPath = "name";
        //                 formatter = that.formatExternalCodeWithName;
        //                 break;

        //             case "3": // Position
        //                 modelName = "PositionModel";
        //                 keyPath = "code";
        //                 textPath = "externalName_en_US";
        //                 formatter = that.formatExternalCodeWithExternalName;
        //                 break;

        //             case "4": // Department
        //                 modelName = "DepartmentModel";
        //                 keyPath = "externalCode";
        //                 textPath = "name";
        //                 formatter = that.formatExternalCodeWithName;
        //                 break;

        //             case "5": // Division
        //                 modelName = "DivisionModel";
        //                 keyPath = "externalCode";
        //                 textPath = "name";
        //                 formatter = that.formatExternalCodeWithName;
        //                 break;

        //             case "6": // Location
        //                 modelName = "LocationModel";
        //                 keyPath = "externalCode";
        //                 textPath = "name";
        //                 formatter = that.formatExternalCodeWithName;
        //                 break;

        //             case "7": // Work Schedule
        //                 modelName = "WorkScheduleModel";
        //                 keyPath = "externalCode";
        //                 textPath = "externalName_en_US";
        //                 formatter = that.formatExternalCodeWithExternalName;
        //                 break;

        //             case "8": // Manager
        //                 modelName = "PositionModel";
        //                 keyPath = "code";
        //                 textPath = "externalName_en_US";
        //                 formatter = that.formatExternalCodeWithExternalName;
        //                 break;

        //             case "9": // Job Title
        //                 modelName = "PositionModel";
        //                 keyPath = "jobCode";
        //                 textPath = "jobTitle";
        //                 formatter = that.formatExternalCodeWithJobTitle;
        //                 break;

        //             default:
        //                 // For any unhandled case, return a simple input
        //                 return new sap.m.Input({
        //                     value: currentStatusValue
        //                 });
        //         }

        //         // Create the ComboBox
        //         comboBox = new sap.m.ComboBox({
        //             placeholder: "Select " + item.Item,
        //             // Important: don't set selectedKey here
        //             items: {
        //                 path: modelName + ">/",
        //                 template: new sap.ui.core.Item({
        //                     key: "{" + modelName + ">" + keyPath + "}",
        //                     text: {
        //                         parts: [
        //                             { path: modelName + ">" + textPath },
        //                             { path: modelName + ">" + keyPath }
        //                         ],
        //                         formatter: formatter
        //                     }
        //                 })
        //             }
        //         });

        //         // Set up a change handler to update the model when user selects a new value
        //         comboBox.attachChange(function(oEvent) {
        //             let selectedItem = oEvent.getParameter("selectedItem");
        //             if (selectedItem) {
        //                 let bindingContext = oEvent.getSource().getBindingContext("DataModel");
        //                 let path = bindingContext.getPath();
        //                 let model = that.getView().getModel("DataModel");
        //                 model.setProperty(path + "/NewStatus", selectedItem.getKey());
        //                 console.log("User selected new value:", selectedItem.getKey());
        //             }
        //         });

        //         // Get the binding to add data received handler
        //         let itemsBinding = comboBox.getBinding("items");

        //         // This is critical: Set up a listener for when data is received
        //         if (itemsBinding) {
        //             // Using a closure to preserve item reference
        //             (function(cBox, cValue, iSNo) {
        //                 itemsBinding.attachEventOnce("dataReceived", function() {
        //                     console.log(`Data received for item ${iSNo}, setting to "${cValue}"`);

        //                     // First attempt - direct selection
        //                     cBox.setSelectedKey(cValue);

        //                     // Second attempt with timeout
        //                     setTimeout(function() {
        //                         // Try setting the selectedKey again
        //                         cBox.setSelectedKey(cValue);

        //                         // If that didn't work, try manual selection
        //                         if (!cBox.getSelectedKey() && cValue) {
        //                             let items = cBox.getItems();
        //                             console.log(`Available items for ${iSNo}:`, items.map(i => `${i.getKey()}`));

        //                             // Look for exact match
        //                             for (let i = 0; i < items.length; i++) {
        //                                 if (items[i].getKey() === cValue) {
        //                                     console.log(`Found exact match for ${iSNo}:`, items[i].getKey());
        //                                     cBox.setSelectedItem(items[i]);
        //                                     break;
        //                                 }
        //                             }

        //                             // If still not set, try string comparison
        //                             if (!cBox.getSelectedKey()) {
        //                                 for (let i = 0; i < items.length; i++) {
        //                                     if (String(items[i].getKey()) === String(cValue)) {
        //                                         console.log(`Found string match for ${iSNo}:`, items[i].getKey());
        //                                         cBox.setSelectedItem(items[i]);
        //                                         break;
        //                                     }
        //                                 }
        //                             }
        //                         }

        //                         console.log(`Final selection for ${iSNo}:`, cBox.getSelectedKey());
        //                     }, 200);
        //                 });
        //             })(comboBox, currentStatusValue, item.SNo);
        //         } else {
        //             console.error("Could not get binding for ComboBox items:", item.SNo);
        //         }

        //         // Also add an event delegate for after rendering
        //         comboBox.addEventDelegate({
        //             onAfterRendering: function() {
        //                 console.log(`ComboBox ${item.SNo} rendered, current selectedKey:`, comboBox.getSelectedKey());

        //                 if (!comboBox.getSelectedKey() && currentStatusValue) {
        //                     // One more attempt to set the key
        //                     setTimeout(function() {
        //                         comboBox.setSelectedKey(currentStatusValue);
        //                         console.log(`After final attempt for ${item.SNo}, selectedKey:`, comboBox.getSelectedKey());
        //                     }, 500);
        //                 }
        //             }
        //         });

        //         return comboBox;
        //     }
        // },



        // bindTableItems: function () {
        //     let that = this;
        //     let table = this.getView().byId("changeStatusTable");

        //     table.bindItems({
        //         path: "DataModel>/ChangeOfStatus",
        //         factory: function (sId, oContext) {
        //             let model = oContext.getModel();
        //             let path = oContext.getPath();
        //             let newStatus = model.getProperty(path + "/NewStatus");
        //             let currentStatus = model.getProperty(path + "/CurrentStatus");

        //             if (!newStatus) {
        //                 model.setProperty(path + "/NewStatus", currentStatus);
        //             }

        //             let cells = [
        //                 new Text({ text: "{DataModel>SNo}" }),
        //                 new Text({ text: "{DataModel>Item}" }),
        //                 new Text({ text: "{DataModel>CurrentStatus}" })
        //             ];

        //             let comboBox;

        //             switch (model.getProperty(path + "/SNo")) {
        //                 case "1":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Company",
        //                         items: {
        //                             path: "CompanyModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{CompanyModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "CompanyModel>name" },
        //                                         { path: "CompanyModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "2":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Cost Center",
        //                         items: {
        //                             path: "CostCenterModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{CostCenterModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "CostCenterModel>name" },
        //                                         { path: "CostCenterModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "3":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Position",
        //                         items: {
        //                             path: "PositionModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{PositionModel>code}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "PositionModel>externalName_en_US" },
        //                                         { path: "PositionModel>code" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithExternalName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "4":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Department",
        //                         items: {
        //                             path: "DepartmentModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{DepartmentModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "DepartmentModel>name" },
        //                                         { path: "DepartmentModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "5":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Division",
        //                         items: {
        //                             path: "DivisionModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{DivisionModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "DivisionModel>name" },
        //                                         { path: "DivisionModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "6":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Location",
        //                         items: {
        //                             path: "LocationModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{LocationModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "LocationModel>name" },
        //                                         { path: "LocationModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "7":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Work Schedule",
        //                         items: {
        //                             path: "WorkScheduleModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{WorkScheduleModel>externalCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "WorkScheduleModel>externalName_en_US" },
        //                                         { path: "WorkScheduleModel>externalCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithExternalName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "8":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Manager",
        //                         items: {
        //                             path: "PositionModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{PositionModel>code}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "PositionModel>externalName_en_US" },
        //                                         { path: "PositionModel>code" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithExternalName
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 case "9":
        //                     comboBox = new ComboBox({
        //                         selectedKey: "{DataModel>NewStatus}",
        //                         placeholder: "Select Job Title",
        //                         items: {
        //                             path: "PositionModel>/",
        //                             template: new sap.ui.core.Item({
        //                                 key: "{PositionModel>jobCode}",
        //                                 text: {
        //                                     parts: [
        //                                         { path: "PositionModel>jobTitle" },
        //                                         { path: "PositionModel>jobCode" }
        //                                     ],
        //                                     formatter: that.formatExternalCodeWithJobTitle
        //                                 }
        //                             })
        //                         }
        //                     });
        //                     break;
        //                 default:
        //                     comboBox = new Input({
        //                         value: "{DataModel>CurrentStatus}"
        //                     });
        //             }

        //             if (comboBox.attachChange) {
        //                 comboBox.attachChange(function (oEvent) {
        //                     let selectedItem = oEvent.getParameter("selectedItem");
        //                     if (selectedItem) {
        //                         let bindingContext = oEvent.getSource().getBindingContext("DataModel");
        //                         let path = bindingContext.getPath();
        //                         let model = oEvent.getSource().getModel("DataModel");
        //                         model.setProperty(path + "/NewStatus", selectedItem.getKey());
        //                     }
        //                 });
        //             }

        //             cells.push(comboBox);

        //             return new ColumnListItem({
        //                 cells: cells
        //             });
        //         }
        //     });
        // },







        initializeView: function () {
            this.initializeChangeOfStatusTable();
            this.fetchCompanyData();
            this.fetchDepartmentData();
            this.fetchDivisionData();
            this.fetchLocationData();
            this.fetchWorkScheduleData();
            this.fetchPositionData();
            this.fetchCostCenterData();
        },

        onUpdatePosition: function () {

            this.byId("submitUpdateButton").setVisible(false);

            this.oButtonModel.setProperty("/showSubmitButton", true);

            let userId = null;

            if (this._selectedItemContext) {
                let selectedRowData = this._selectedItemContext.getObject();
                userId = selectedRowData.externalCode;
            } else if (this.oSelectedRowModel && this.oSelectedRowModel.getData().selectedRow) {
                userId = this.oSelectedRowModel.getData().selectedRow.externalCode;
            }

            if (!userId) {
                MessageToast.show("No employee selected. Please select an employee first.");
                console.error("No user ID available for onUpdatePosition");
                this.byId("submitUpdateButton").setVisible(true);
                return;
            }

            console.log("Updating position for user:", userId);

            // Fetch employee data with the selected user ID
            this.fetchEmpJobData(userId);
            this.fetchEmpCompensationData(userId);
            this.fetchRequestApprovalData(userId);
            this._getEventReasons();
            this._getWorkflowDetails(userId);
            // this.highlightChangeOfStatusSection();
            // this.highlightChangeOfCompSection();


            // Initialize view after data is fetched
            if (this.empJobData) {
                this.initializeView();
            } else {
                MessageToast.show("Failed to load employee data. Please try again.");
                this.byId("submitUpdateButton").setVisible(true);
            }
        },



        // highlightChangeOfStatusSection: function () {
        //     let objectPage = this.getView().byId("ObjectPageLayout"); // Corrected ID
        //     let changeOfStatusSection = this.getView().byId("changeOfStatusSection");

        //     if (changeOfStatusSection) {
        //         changeOfStatusSection.setVisible(true);

        //         if (objectPage) {

        //             setTimeout(function () {
        //                 objectPage.scrollToSection(changeOfStatusSection.getId(), 500);
        //             }, 0); // Short delay
        //         }
        //     }
        // },

        // highlightChangeOfCompSection: function () {
        //     let objectPage = this.getView().byId("ObjectPageLayout"); // Corrected ID
        //     let changeOfStatusSection = this.getView().byId("changeOfCompSection");

        //     if (changeOfStatusSection) {
        //         changeOfStatusSection.setVisible(true);

        //         if (objectPage) {
        //             setTimeout(function () {
        //                 objectPage.scrollToSection(changeOfStatusSection.getId(), 500);
        //             }, 0);
        //         }
        //     }
        // },

        highlightSubmitApprovalsSection: function () {
            let objectPage = this.getView().byId("ObjectPageLayout"); // Corrected ID
            let submitApprovalsSection = this.getView().byId("submitApprovalsSection");

            if (submitApprovalsSection) {
                submitApprovalsSection.setVisible(true);

                if (objectPage) {

                    setTimeout(function () { // Delay to ensure previous scroll is done
                        objectPage.scrollToSection(submitApprovalsSection.getId(), 500); // Scroll to section
                    }, 0); // Short delay
                }
            }
        },



        disableChangeOfStatusSection: function () {
            let changeOfStatusSection = this.getView().byId("changeOfStatusSection");

            if (changeOfStatusSection) {
                changeOfStatusSection.setVisible(false);
            }
        },

        disableChangeOfCompSection: function () {
            let changeOfStatusSection = this.getView().byId("changeOfCompSection");

            if (changeOfStatusSection) {
                changeOfStatusSection.setVisible(false);
            }
        },

        disableSubmitApprovalsSection: function () {
            let submitApprovalsSection = this.getView().byId("submitApprovalsSection");

            if (submitApprovalsSection) {
                submitApprovalsSection.setVisible(false);
            }
        },

        convertToODataDate: function (dateString) {
            if (!dateString) return "/Date(" + Date.now() + ")/";

            try {
                let oDate = new Date(dateString);
                return "/Date(" + oDate.getTime() + ")/";
            } catch (error) {
                console.error("Date conversion error:", error);
                return "/Date(" + Date.now() + ")/";
            }
        },

        onNewButtonPress: function () {


            if (!this._oNewRequestDialog) {
                let oView = this.getView();

                Fragment.load({
                    id: oView.getId(),
                    name: "com.taqa.psnform.taqapsnform.view.New",
                    controller: this
                }).then(function (oDialog) {
                    oView.addDependent(oDialog);
                    this._oNewRequestDialog = oDialog;
                    this._oNewRequestDialog.open();
                }.bind(this));
            } else {
                // If already created, just open it
                this._oNewRequestDialog.open();
            }
        },

        onCloseNewDialog: function () {
            if (this._oNewRequestDialog) {
                this._oNewRequestDialog.close();
            }


        },

        // // important function

        // onSubmitPosition: function () {
        //     let that = this;

        //     let oDataModel = this.getView().getModel("DataModel");

        //     // Check if DataModel exists
        //     if (!oDataModel) {
        //         sap.m.MessageBox.error("Data model not found. Please refresh the page and try again.");
        //         return;
        //     }

        //     let aChangeOfStatus = oDataModel.getProperty("/ChangeOfStatus");

        //     // Check if ChangeOfStatus exists in the model
        //     if (!aChangeOfStatus || !Array.isArray(aChangeOfStatus)) {
        //         sap.m.MessageBox.error("Position data not loaded correctly. Please refresh the page and try again.");
        //         return;
        //     }

        //     // Continue with the existing code...
        //     let oJobTitleRow = aChangeOfStatus.find(function (row) {
        //         return row.Item === "Job Title";
        //     });

        //     let sSelectedJobTitle = oJobTitleRow ? oJobTitleRow.NewStatus : "";

        //     if (!sSelectedJobTitle) {
        //         sap.m.MessageToast.show("Please select a job title");
        //         return;
        //     }

        //     // Check if an event reason has been selected
        //     if (!this._selectedEventReason) {
        //         sap.m.MessageToast.show("Please select an event reason");
        //         return;
        //     }

        //     this._getUserIdAndOpenAddComponentDialog()
        //         .then(function () {
        //             let sUserId = that._currentUserId;
        //             let sEffectiveDate = that._currentEffectiveDate;
        //             let sFormattedDate = "/Date(" + new Date(sEffectiveDate).getTime() + ")/";

        //             // Get the event reason from the stored property
        //             let sEventReason = that._selectedEventReason ? that._selectedEventReason.externalCode : "";




        //             let oPayloadData = {
        //                 "__metadata": {
        //                     "uri": "EmpJob"
        //                 },
        //                 "userId": sUserId,
        //                 "eventReason": sEventReason,
        //                 "startDate": sFormattedDate,
        //                 "jobTitle": sSelectedJobTitle
        //             };

        //             let oBusyDialog = new sap.m.BusyDialog();
        //             oBusyDialog.open();

        //             let sUrl = that.getPath("SF_2") + "/upsert";

        //             $.ajax({
        //                 url: sUrl,
        //                 type: "POST",
        //                 contentType: "application/json",
        //                 data: JSON.stringify(oPayloadData),


        //                 success: function (data) {
        //                     oBusyDialog.close();
        //                     console.log("Raw Response:", data);

        //                     let oXmlDoc;
        //                     if (typeof data === "string") {
        //                         let oParser = new DOMParser();
        //                         oXmlDoc = oParser.parseFromString(data, "text/xml");
        //                     } else {
        //                         oXmlDoc = data;
        //                     }

        //                     function getValueFromXML(doc, tagName) {
        //                         let nodes = doc.getElementsByTagName(tagName);
        //                         return nodes.length > 0 ? nodes[0].textContent : "";
        //                     }

        //                     let sKey = getValueFromXML(oXmlDoc, "d:key");
        //                     let sStatus = getValueFromXML(oXmlDoc, "d:status");
        //                     let sEditStatus = getValueFromXML(oXmlDoc, "d:editStatus");
        //                     let sMessage = getValueFromXML(oXmlDoc, "d:message");

        //                     let oResponseObject = {
        //                         key: sKey,
        //                         status: sStatus,
        //                         editStatus: sEditStatus,
        //                         message: sMessage
        //                     };

        //                     let oResponseModel = new sap.ui.model.json.JSONModel(oResponseObject);
        //                     that.getView().setModel(oResponseModel, "responseModel");

        //                     // sap.m.MessageToast.show("Status: " + sMessage);

        //                     sap.m.MessageBox.show("Status: " + sMessage, {
        //                         title: "Response Info",
        //                         actions: [sap.m.MessageBox.Action.OK],
        //                         onClose: function (oAction) {
        //                             if (oAction === sap.m.MessageBox.Action.OK) {
        //                                 // Hide both update and submit buttons when OK is clicked
        //                                 that.byId("submitUpdateButton").setVisible(false);
        //                                 that.oButtonModel.setProperty("/showSubmitButton", false);

        //                                 let sWorkflowUrl = that.getPath("SF_1") +
        //                                     "/EmpWfRequest?$format=json&$filter=subjectId eq '" + sUserId + "' and requestType eq 'CHANGE_JOB'&$select=wfRequestId&$top=1";

        //                                 jQuery.ajax({
        //                                     url: sWorkflowUrl,
        //                                     method: "GET",
        //                                     dataType: "json",
        //                                     async: false,
        //                                     headers: {
        //                                         "Accept": "application/json"
        //                                     },
        //                                     success: function (data) {
        //                                         if (data && data.d && data.d.results) {
        //                                             console.log("Available Event Reasons for JOBINFO:", data.d.results);
        //  that.highlightSubmitApprovalsSection();
        //                                             that._getWorkflowDetails(sUserId);
        //                                         }

        //                                         console.log("Checking the GET call")
        //                                     },
        //                                     error: function (e) {
        //                                         sap.m.MessageToast.show("Error fetching department data");
        //                                         console.error("Error fetching department data:", e);
        //                                     }
        //                                 });
        //                             }
        //                         }
        //                     });
        //                 },

        //                 error: function (jqXHR, textStatus, errorThrown) {
        //                     oBusyDialog.close();

        //                     let sErrorMessage = "Failed to Submit Position Change";
        //                     try {
        //                         let oErrorResponse = JSON.parse(jqXHR.responseText);
        //                         if (oErrorResponse && oErrorResponse.message) {
        //                             sErrorMessage = oErrorResponse.message || sErrorMessage;
        //                         }
        //                     } catch (e) {
        //                         if (errorThrown) {
        //                             sErrorMessage += ": " + errorThrown;
        //                         }
        //                     }

        //                     sap.m.MessageBox.error(sErrorMessage);

        //                 }
        //             });
        //         })

        //         .catch(function (error) {
        //             sap.m.MessageBox.error("Could not submit: " + error.message);

        //         });
        // },


        onSubmitPosition: function () {
            let that = this;

            let oDataModel = this.getView().getModel("DataModel");
            let oCompensationModel = this.getView().getModel("ChangeOfCompensationModel");

            // Check if DataModel exists
            if (!oDataModel) {
                MessageBox.error("Data model not found. Please refresh the page and try again.");
                return;
            }

            let aChangeOfStatus = oDataModel.getProperty("/ChangeOfStatus");

            // Check if ChangeOfStatus exists in the model
            if (!aChangeOfStatus || !Array.isArray(aChangeOfStatus)) {
                MessageBox.error("Position data not loaded correctly. Please refresh the page and try again.");
                return;
            }

            // Check if any changes have been made to position
            let bHasPositionChanges = aChangeOfStatus.some(function (item) {
                return item.NewStatus && item.NewStatus !== item.CurrentStatus;
            });

            // Check if any changes have been made to compensation
            let bHasCompensationChanges = false;
            let aCompensationChanges = [];

            if (oCompensationModel) {
                let aChangeOfCompensation = oCompensationModel.getProperty("/ChangeOfCompensation");
                if (aChangeOfCompensation && Array.isArray(aChangeOfCompensation)) {
                    aCompensationChanges = aChangeOfCompensation.filter(function (item) {
                        return item.NewValue && item.NewValue !== "";
                    });
                    bHasCompensationChanges = aCompensationChanges.length > 0;
                }
            }

            // Check if there are any changes at all
            if (!bHasPositionChanges && !bHasCompensationChanges) {
                MessageToast.show("No changes detected. Please make changes before submitting.");
                return;
            }

            //             // Check if an event reason has been selected
            // if (!this._selectedEventReason || !this._selectedEventReason.externalCode) {
            //     sap.m.MessageToast.show("Please select an event reason");
            //     return;
            // }

            // // Debug log to check what's in the _selectedEventReason
            // console.log("Event reason before submission:", this._selectedEventReason);



            this._getUserIdAndOpenAddComponentDialog()
                .then(function () {
                    let sUserId = that._currentUserId;
                    let sEffectiveDate = that._currentEffectiveDate;
                    let sFormattedDate = "/Date(" + new Date(sEffectiveDate).getTime() + ")/";
                    let sEventReason = that._currentPsnTypeChange || "";
                    console.log("Current PSN Type Change:", that._currentPsnTypeChange);

                    //     // Get the event reason from the stored property
                    //     // let sEventReason = that._currentPsnTypeChange ;

                    //    let sEventReason = that._selectedEventReason ? that._selectedEventReason.externalCode : "";

                    // Create base payload for position changes
                    let oPositionPayload = {
                        "__metadata": {
                            "uri": "EmpJob"
                        },
                        "userId": sUserId,
                        "eventReason": sEventReason,
                        "startDate": sFormattedDate
                    };

                    // Add only changed fields to the position payload
                    aChangeOfStatus.forEach(function (item) {
                        if (item.NewStatus && item.NewStatus !== item.CurrentStatus) {
                            switch (item.Item) {
                                case "Company":
                                    oPositionPayload.company = item.NewStatus;
                                    break;
                                case "Cost Center":
                                    oPositionPayload.costCenter = item.NewStatus;
                                    break;
                                case "Position":
                                    oPositionPayload.position = item.NewStatus;
                                    break;
                                case "Department":
                                    oPositionPayload.department = item.NewStatus;
                                    break;
                                case "Division":
                                    oPositionPayload.division = item.NewStatus;
                                    break;
                                case "Location":
                                    oPositionPayload.location = item.NewStatus;

                                    let oLocationModel = that.getView().getModel("LocationModel");
                                    if (oLocationModel) {
                                        let aLocations = oLocationModel.getProperty("/");
                                        if (Array.isArray(aLocations)) {
                                            let oSelectedLocation = aLocations.find(function (loc) {
                                                return loc.locationId === item.NewStatus || loc.externalCode === item.NewStatus;
                                            });

                                            if (oSelectedLocation && oSelectedLocation.locationGroup) {
                                                oPositionPayload.customString7 = oSelectedLocation.locationGroup;
                                            } else {
                                                oPositionPayload.customString7 = ""; // Or a default value if necessary
                                            }
                                        }
                                    }
                                    break;

                                case "Work Schedule":
                                    oPositionPayload.workscheduleCode = item.NewStatus;
                                    break;
                                case "Manager":
                                    oPositionPayload.managerId = item.NewStatus;
                                    break;
                                case "Job Title":
                                    oPositionPayload.jobTitle = item.NewStatus;
                                    break;
                            }
                        }
                    });

                    // Log the position payload for debugging
                    console.log("Position Payload for submission:", oPositionPayload);

                    let oBusyDialog = new sap.m.BusyDialog();
                    oBusyDialog.open();

                    // Helper function to make AJAX requests with timeout and retry capability
                    function makeAjaxRequest(url, payload) {
                        return new Promise(function (resolve, reject) {
                            $.ajax({
                                url: url,
                                type: "POST",
                                contentType: "application/json",
                                data: JSON.stringify(payload),
                                timeout: 120000, // 2-minute timeout
                                success: function (data) {
                                    resolve(data);
                                },
                                error: function (jqXHR, textStatus, errorThrown) {
                                    reject({
                                        error: jqXHR,
                                        textStatus: textStatus,
                                        errorThrown: errorThrown,
                                        payload: payload
                                    });
                                }
                            });
                        });
                    }
                    async function processRequestsSequentially() {
                        let results = [];
                        let sUrl = that.getPath("SF_2") + "/upsert";

                        try {
                            // First process position changes if any
                            if (bHasPositionChanges) {
                                console.log("Processing position changes...");
                                let positionData = await makeAjaxRequest(sUrl, oPositionPayload);
                                results.push({ type: "position", data: positionData });
                            }

                            // Then process grade change if any
                            if (bHasCompensationChanges) {
                                let gradeItem = aCompensationChanges.find(function (item) {
                                    return item.SNo === "1";
                                });

                                if (gradeItem && gradeItem.NewValue) {
                                    console.log("Processing grade change...");
                                    let oGradePayload = {
                                        "__metadata": {
                                            "uri": "EmpJob"
                                        },
                                        "userId": sUserId,
                                        "startDate": sFormattedDate,
                                        "payGrade": gradeItem.NewValue
                                    };

                                    let gradeData = await makeAjaxRequest(sUrl, oGradePayload);
                                    results.push({ type: "grade", data: gradeData, item: gradeItem });
                                }

                                // Then process each compensation component one by one
                                for (let i = 0; i < aCompensationChanges.length; i++) {
                                    let item = aCompensationChanges[i];
                                    // Skip the Grade item as it's handled separately
                                    if (item.SNo !== "1") {
                                        console.log("Processing compensation item:", item.Item);
                                        let oCompPayload = {
                                            "__metadata": {
                                                "uri": "EmpPayCompRecurring"
                                            },
                                            "userId": sUserId,
                                            "startDate": sFormattedDate,
                                            "payComponent": item.externalCode,
                                            "paycompvalue": item.NewValue
                                        };

                                        let compData = await makeAjaxRequest(sUrl, oCompPayload);
                                        results.push({ type: "compensation", data: compData, item: item });
                                    }
                                }
                            }

                            return results;
                        } catch (error) {
                            throw error;
                        }
                    }

                    // Execute the sequential processing
                    processRequestsSequentially()
                        .then(function (results) {
                            oBusyDialog.close();

                            console.log("All submission results:", results);

                            if (results.length === 0) {
                                MessageBox.information("No changes were processed.");
                                return;
                            }

                            // Process the first result for display
                            let firstResult = results[0];
                            let data = firstResult.data;

                            let oXmlDoc;
                            if (typeof data === "string") {
                                let oParser = new DOMParser();
                                oXmlDoc = oParser.parseFromString(data, "text/xml");
                            } else {
                                oXmlDoc = data;
                            }

                            function getValueFromXML(doc, tagName) {
                                let nodes = doc.getElementsByTagName(tagName);
                                return nodes.length > 0 ? nodes[0].textContent : "";
                            }

                            let sKey = getValueFromXML(oXmlDoc, "d:key");
                            let sStatus = getValueFromXML(oXmlDoc, "d:status");
                            let sEditStatus = getValueFromXML(oXmlDoc, "d:editStatus");
                            let sMessage = getValueFromXML(oXmlDoc, "d:message");

                            let oResponseObject = {
                                key: sKey,
                                status: sStatus,
                                editStatus: sEditStatus,
                                message: sMessage
                            };

                            let oResponseModel = new sap.ui.model.json.JSONModel(oResponseObject);
                            that.getView().setModel(oResponseModel, "responseModel");

                            // Determine overall message
                            let sSuccessCount = results.length;
                            let sOverallMessage = sSuccessCount + " changes submitted successfully. " + sMessage;

                            MessageBox.show(sOverallMessage, {
                                title: "Response Info",
                                actions: [MessageBox.Action.OK],
                                onClose: function (oAction) {
                                    if (oAction === MessageBox.Action.OK) {
                                        // Hide both update and submit buttons when OK is clicked
                                        that.byId("submitUpdateButton").setVisible(false);
                                        that.oButtonModel.setProperty("/showSubmitButton", false);

                                        let sWorkflowUrl = that.getPath("SF_1") +
                                            "/EmpWfRequest?$format=json&$filter=subjectId eq '" + sUserId + "' and requestType eq 'CHANGE_JOB'&$select=wfRequestId&$top=1";

                                        jQuery.ajax({
                                            url: sWorkflowUrl,
                                            method: "GET",
                                            dataType: "json",
                                            async: false,
                                            headers: {
                                                "Accept": "application/json"
                                            },
                                            success: function (data) {
                                                if (data && data.d && data.d.results) {
                                                    console.log("Available Event Reasons for JOBINFO:", data.d.results);
                                                    that.highlightSubmitApprovalsSection();
                                                    that._getWorkflowDetails(sUserId);
                                                }

                                                console.log("Checking the GET call");
                                            },
                                            error: function (e) {
                                                MessageToast.show("Error fetching workflow data");
                                                console.error("Error fetching workflow data:", e);
                                            }
                                        });
                                    }
                                }
                            });
                        })
                        .catch(function (error) {
                            oBusyDialog.close();

                            let sErrorMessage = "Failed to Submit Change";

                            if (error.error) {
                                try {
                                    let oErrorResponse = JSON.parse(error.error.responseText);
                                    if (oErrorResponse && oErrorResponse.message) {
                                        sErrorMessage = oErrorResponse.message || sErrorMessage;
                                    }
                                } catch (e) {
                                    if (error.errorThrown) {
                                        sErrorMessage += ": " + error.errorThrown;
                                    }
                                }
                            }

                            if (error.payload) {
                                if (error.payload.__metadata && error.payload.__metadata.uri === "EmpPayCompRecurring") {
                                    sErrorMessage = "Failed to update compensation: " + sErrorMessage;
                                } else if (error.payload.payGrade) {
                                    sErrorMessage = "Failed to update grade: " + sErrorMessage;
                                }
                            }

                            MessageBox.error(sErrorMessage);
                        });
                })
                .catch(function (error) {
                    MessageBox.error("Could not submit: " + error.message);
                });
        },

        _getWorkflowDetails: function (userId) {
            let that = this;

            let sListUrl = this.getPath("SF_1") +
                "/EmpWfRequest?$format=json&$filter=subjectId eq '" + userId + "' and requestType eq 'CHANGE_JOB'" +
                "&$select=wfRequestId&$top=1";

            $.ajax({
                url: sListUrl,
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
                    let aResults = data.d && data.d.results;

                    if (Array.isArray(aResults)) {
                        aResults.forEach(function (item, index) {
                            let wfRequestId = item.wfRequestId;

                            if (wfRequestId) {
                                let sDetailUrl = that.getPath("SF_1") +
                                    `/WfRequest(${wfRequestId}L)?$format=json&$expand=wfRequestStepNav,wfRequestStepNav/positionNav&$select=wfRequestId,totalSteps,currentStepNum,status,wfRequestStepNav/stepNum,wfRequestStepNav/wfRequestStepId,wfRequestStepNav/status,wfRequestStepNav/positionNav/code,wfRequestStepNav/positionNav/externalName_en_US`;

                                $.ajax({
                                    url: sDetailUrl,
                                    type: "GET",
                                    dataType: "json",
                                    async: false,
                                    success: function (oDetailData) {
                                        let wfDataModel = new sap.ui.model.json.JSONModel(oDetailData.d);
                                        let sModelName = "wfData_" + index;
                                        that.getView().setModel(wfDataModel, sModelName);
                                        console.log("Workflow model set for:", wfRequestId, oDetailData.d);
                                    },
                                    error: function (xhr, status, error) {
                                        console.error("Error fetching workflow detail for ID:", wfRequestId);
                                        console.error("Status:", status);
                                        console.error("Error:", error);
                                        console.error("Response Text:", xhr.responseText);
                                        MessageToast.show("Error fetching workflow detail for ID: " + wfRequestId);
                                    }
                                });
                            }
                        });
                    }
                },
                error: function () {
                    MessageToast.show("Error fetching workflow request list");
                }
            });
        },

        onGeneratePDF: async function () {
            // Sample dynamic data – replace with real model bindings
            const employeeName = "John Doe"; // get from model
            const positionTitle = "Senior Developer"; // get from model
            const roleName = "Leading the Frontend Team"; // get from model
            const changeDate = "2025-06-01"; // get from model
            const amount = "AED 10,000"; // get from model
            const benefits = "Private Medical, Annual Bonus, Housing Allowance"; // get from model
            const managerName = "Jane Smith"; // get from model
        
            const currentDate = new Date().toLocaleDateString('en-GB');
        
            const docText = 
        `<TAQA>
        <UAE>
        ${currentDate}
        
        Dear ${employeeName},
        
        It is with great pleasure that I inform you of your well-deserved promotion to ${positionTitle}. Your outstanding performance, dedication, and commitment to our organization have not gone unnoticed, and we are delighted to recognize your hard work with this promotion.
        
        Your new role will involve ${roleName}. We have no doubt that you will excel in this new position, bringing your expertise and enthusiasm to the role and contributing to the continued success of our organization.
        
        Effective ${changeDate}, you will receive a salary increase of ${amount} in recognition of your new role and increased responsibilities. You will also be eligible for additional benefits, including ${benefits}.
        
        We understand that this is an exciting but challenging time for you. We want to assure you that we are committed to supporting you in your new role and providing you with the necessary resources to succeed. We have every confidence that you will rise to the challenge and deliver excellent results in your new position.
        
        Please acknowledge your acceptance of this promotion in writing by ${currentDate}. If you have any questions or concerns about your new role, please do not hesitate to contact me or ${managerName}.
        
        Congratulations once again on your well-deserved promotion. We look forward to your continued success and contributions to our organization.
        
        Sincerely,
        
        ${employeeName}
        <TAQA>`;
        
            // Load jsPDF from CDN if needed
            if (!window.jspdf) {
                await new Promise((resolve) => {
                    const script = document.createElement("script");
                    script.src = "https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";
                    script.onload = resolve;
                    document.head.appendChild(script);
                });
            }
        
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();
        
            const splitText = doc.splitTextToSize(docText, 180);
            doc.setFont("Times", "Normal");
            doc.setFontSize(12);
            doc.text(splitText, 10, 20);
            
            const filename = `Promotion_Letter_${employeeName.replace(/\s+/g, "_")}.pdf`;
            doc.save(filename);
        }
        

    });





});